...
for snpfilter_dir in $(ls -d */); do
	if [ -d "$snpfilter_dir" ]; then
		...
		for i in $(ls *dose.txt 2> /dev/null); do
			...
			Rscript "${GBSapp_dir}"/scripts/R/heterozygote_vs_allele_ratio.R "$i" "$ARfile" "ploidy" "3" "${GBSapp_dir}/tools/R"
		done
		wait
		...
		for i in $(ls *dose_unique_mapped.txt 2> /dev/null); do
			ARselect=${i%rd*}
			ARfile=$(ls ../../snpcall/${ARselect}*_AR.txt 2> /dev/null)
			Rscript "${GBSapp_dir}"/scripts/R/heterozygote_vs_allele_ratio_uniqfiltered.R "$i" "$ARfile" "ploidy" "3" "${GBSapp_dir}/tools/R"
		done
		wait
		for i in $(ls *dose_multi_mapped.txt 2> /dev/null); do
			ARselect=${i%rd*}
			ARfile=$(ls ../../snpcall/${ARselect}*_AR.txt 2> /dev/null)
			Rscript "${GBSapp_dir}"/scripts/R/heterozygote_vs_allele_ratio_multifiltered.R "$i" "$ARfile" "ploidy" "3" "${GBSapp_dir}/tools/R"
		done
		wait
