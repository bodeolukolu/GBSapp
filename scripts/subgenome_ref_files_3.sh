	...
	if [[ "${joint_alignment}" -eq 1 ]]; then
		  ...
		  for i in $(cat ${projdir}/${samples_list} ); do (
		    ...
		    if test ! -f ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_precall.bam.bai && test ! -f ${projdir}/preprocess/${i%.f*}_${ref2%.f*}_precall.bam.bai && test ! -f ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_precall.bam.bai; then
		      ...
		      awk '{if($1==3) print $2}' ${i%.f*}_Index_subgenome.txt | grep -Fw -f - <(zcat ${i%.f*}_redun.sam.gz 2> /dev/null) | cat <(zcat ${i%.f*}_redun.sam.gz 2> /dev/null | grep '^@') - > ${i%.f*}_del_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.sam
		      ...
		      awk -v ref3=${ref3%.f*} -F '\t' '$3 ~ ref3{print $1}' ${i%.f*}_del0_${ref1%.f*}_${ref2%.f*}.sam | \
		      ...
		      awk '{if($1==2) print $0}' ${i%.f*}_Index_subgenome.txt | awk -v ref1=${ref1%.f*} -v ref2=${ref3%.f*} '{if($3 == ref1 || $3 == ref3) print $2}' | grep -Fw -f - <(zcat ${i%.f*}_redun.sam.gz 2> /dev/null) > ${i%.f*}_del0_${ref1%.f*}_${ref3%.f*}.sam
		      awk -v ref2=${ref2%.f*} -F '\t' '$3 ~ ref2{print $1}' ${i%.f*}_del0_${ref1%.f*}_${ref3%.f*}.sam | \
		      ...
		      awk '{if($1==2) print $0}' ${i%.f*}_Index_subgenome.txt | awk -v ref1=${ref2%.f*} -v ref2=${ref3%.f*} '{if($3 == ref2 || $3 == ref3) print $2}' | grep -Fw -f - <(zcat ${i%.f*}_redun.sam.gz 2> /dev/null) > ${i%.f*}_del0_${ref2%.f*}_${ref3%.f*}.sam
		      awk -v ref1=${ref1%.f*} -F '\t' '$3 ~ ref1{print $1}' ${i%.f*}_del0_${ref2%.f*}_${ref3%.f*}.sam | \
...
main () {
    ...
    for i in $(cat ${projdir}/${samples_list} ); do (
    	...
    	if test ! -f "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz"; then
    		if test ! -f "${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf.gz"; then
    			if [[ -z "$Get_Chromosome" ]]; then
    				$GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --dont-use-soft-clipped-bases $softclip --max-reads-per-alignment-start $downsample --minimum-mapping-quality 10 --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
    				wait
    			else
    				...
    				$GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/panref.list -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --dont-use-soft-clipped-bases $softclip --max-reads-per-alignment-start $downsample --minimum-mapping-quality 10 --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
    				wait
    			fi
    ...
    if [[ "$samples_list" == "samples_list_node_1.txt" ]] && test ! -f ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf*; then
    	...
    	if [[ $call123 == $nodes ]]; then
    		...
    		for dir in cohorts*/; do
    			...
    			for selchr in $Get2_Chromosome; do
    				$GATK --java-options "$Xmxp -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$threads" GenomicsDBImport ${input} -L ${selchr} --genomicsdb-workspace-path ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw
						wait
    			done
    			for selchr in $Get2_Chromosome; do (
    				if test ! -f "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz"; then
    					$GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/panref.fasta -L ${selchr} -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.hold.vcf.gz && \
    ...
    for i in $(cat ${projdir}/${samples_list} ); do (
    	...
    	if test ! -f "${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_raw.vcf.gz"; then
    ...
    for i in $(cat ${projdir}/${samples_list} ); do (
    	...
    	if test ! -f "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf.gz"; then
    ...
    for i in $(cat ${projdir}/${samples_list} ); do (
    	...
    	if test ! -f "${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf.gz"; then
    ...
    ######################
			...
			for i in $(cat ${projdir}/${samples_list} ); do (
				...
				if test ! -f "${pop}_${ref1%.f*}_${ploidy_ref1}x_raw.vcf.gz"; then
		...
		######################
			...
			for i in $(cat ${projdir}/${samples_list} ); do (
				...
				if test ! -f "${pop}_${ref2%.f*}_${ploidy_ref2}x_raw.vcf.gz"; then
    ...
    ######################
			...
			for i in $(cat ${projdir}/${samples_list} ); do (
				...
				if test ! -f "${pop}_${ref3%.f*}_${ploidy_ref3}x_raw.vcf.gz"; then
...
if [[ "$ploidy" -eq 1 ]] || [[ "$ploidy_ref1" -eq 1 ]] || [[ "$ploidy_ref2" -eq 1 ]] || [[ "$ploidy_ref3" -eq 1 ]]; then
	mkdir 1x
fi
if [[ "$ploidy" -eq 2 ]] || [[ "$ploidy_ref1" -eq 2 ]] || [[ "$ploidy_ref2" -eq 2 ]] || [[ "$ploidy_ref3" -eq 2 ]]; then
	mkdir 2x
fi
if [[ "$ploidy" -eq 4 ]] || [[ "$ploidy_ref1" -eq 4 ]] || [[ "$ploidy_ref2" -eq 4 ]] || [[ "$ploidy_ref3" -eq 4 ]]; then
	mkdir 4x
fi
if [[ "$ploidy" -eq 6 ]] || [[ "$ploidy_ref1" -eq 6 ]] || [[ "$ploidy_ref2" -eq 6 ]] || [[ "$ploidy_ref3" -eq 6 ]]; then
	mkdir 6x
fi
if [[ "$ploidy" -eq 8 ]] || [[ "$ploidy_ref1" -eq 8 ]] || [[ "$ploidy_ref2" -eq 8 ]] || [[ "$ploidy_ref3" -eq 8 ]]; then
	mkdir 8x
fi
...
for snpfilter_dir in $(ls -d */); do
	if [ -d "$snpfilter_dir" ]; then
		...
		for i in $(ls *dose.txt 2> /dev/null); do
			...
			Rscript "${GBSapp_dir}"/scripts/R/heterozygote_vs_allele_ratio.R "$i" "$ARfile" "${ploidydir}x" "3" "${GBSapp_dir}/tools/R"
		...
		for i in $(ls *dose_unique_mapped.txt 2> /dev/null); do
			ARselect=${i%rd*}
			ARfile=$(ls ../../../snpcall/${ARselect}*AR.txt 2> /dev/null)
			Rscript "${GBSapp_dir}"/scripts/R/heterozygote_vs_allele_ratio_uniqfiltered.R "$i" "$ARfile" "${ploidydir}x" "3" "${GBSapp_dir}/tools/R"
			awk 'FNR==NR{a[$1,$2]=$0;next}{if(b=a[$2,$3]){print b}}' $ARfile $i | awk '{gsub(/NA/,"na"); print $1"_"$2"\t"$0}' | \
			awk 'gsub(/CHROM_POS/,"SNP");}1' > ${i%.txt}_AR_metric.txt
		...
		for i in $(ls *dose_multi_mapped.txt 2> /dev/null); do
			ARselect=${i%rd*}
			ARfile=$(ls ../../../snpcall/${ARselect}*AR.txt 2> /dev/null)
			Rscript "${GBSapp_dir}"/scripts/R/heterozygote_vs_allele_ratio_multifiltered.R "$i" "$ARfile" "${ploidydir}x" "3" "${GBSapp_dir}/tools/R"
			awk 'FNR==NR{a[$1,$2]=$0;next}{if(b=a[$2,$3]){print b}}' $ARfile $i | awk '{gsub(/NA/,"na"); print $1"_"$2"\t"$0}' | \
			awk 'gsub(/CHROM_POS/,"SNP");}1' > ${i%.txt}_AR_metric.txt
