    ...
    if [[ "$samples_list" == "samples_list_node_1.txt" ]] && test ! -f ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf*; then
    	...
    	if [[ $call123 == $nodes ]]; then
    		...
    		for dir in cohorts*/; do
    			...
    			for selchr in $Get2_Chromosome; do
    				$GATK --java-options "$Xmxp -XX:+UseParallelGC -XX:ParallelGCThreads=$threads" GenomicsDBImport ${input} -L ${selchr} --genomicsdb-workspace-path ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw
						wait
    			done
    			for selchr in $Get2_Chromosome; do (
    				if test ! -f "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz"; then
    					$GATK --java-options "$Xmx1 -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/panref.fasta -L ${selchr} -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.hold.vcf.gz && \
