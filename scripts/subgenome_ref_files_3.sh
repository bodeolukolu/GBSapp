...
if [ "$variant_caller" == "bcftools" ]; then
 ...
 export ploidy_ref2=2
 export ploidy_ref3=2
fi
if [ -z "$lib_type" ]; then
 export lib_type=RRS
fi
...
if [[ "$gap_split_align" == "true" ]]; then
  if [[ $(ls ./*.mmi 1> /dev/null 2>&1 | wc -l) -gt 0 ]]; then
    ...
    $samtools faidx $ref3
    $java -jar $picard CreateSequenceDictionary REFERENCE=$ref3 OUTPUT=${ref3%.f*}.dict
    $minimap2 -d ${ref3%.f*}.mmi $ref3
  fi
fi
wait
if [[ "$RNA" == "true" ]]; then
  mkdir -p star_index_${ref1%.f*} star_index_${ref2%.f*}
  if [[ "$(ls ./star_index_${ref1%.f*}/* 1> /dev/null 2>&1 | wc -l)" -eq 0 ]]; then
    gff=${ref1%.fasta}.gff*
    "$star" --runThreadN $threads \
         --runMode genomeGenerate \
         --genomeDir star_index_${ref1%.f*} \
         --genomeFastaFiles $ref1 \
         --sjdbGTFfile $gff \
         --sjdbOverhang 136
  fi
  if [[ "$(ls ./star_index_${ref2%.f*}/* 1> /dev/null 2>&1 | wc -l)" -eq 0 ]]; then
    gff=${ref2%.fasta}.gff*
    "$star" --runThreadN $threads \
         --runMode genomeGenerate \
         --genomeDir star_index_${ref2%.f*} \
         --genomeFastaFiles $ref2 \
         --sjdbGTFfile $gff \
         --sjdbOverhang 136
  fi
  if [[ "$(ls ./star_index_${ref3%.f*}/* 1> /dev/null 2>&1 | wc -l)" -eq 0 ]]; then
    gff=${ref3%.fasta}.gff*
    "$star" --runThreadN $threads \
         --runMode genomeGenerate \
         --genomeDir star_index_${ref3%.f*} \
         --genomeFastaFiles $ref3 \
         --sjdbGTFfile $gff \
         --sjdbOverhang 136
  fi
fi
...
main () {
  ...
  if [[ $nodes -gt 1 ]]; then
      ...
      if [[ "$gap_split_align" == "false" ]]; then
        if test ! -f ${projdir}/precall_done.txt && test ! -f ${projdir}/alignment_done; then
          while IFS="" read -r alignfq || [ -n "$alignfq" ]; do
            sleep $((RANDOM % 2))
            if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref1%.f*}.sam.gz; then
              $ngm -r ../refgenomes/$ref1 --qry ${alignfq%.f*}_uniq.fasta.gz -o ../preprocess/alignment/${alignfq%.f*}_redun.sam -R 20 -t $threads --topn 8 --strata 8 --affine &&
              ...
              cat ../preprocess/alignment/${alignfq%.f*}_redun_head.sam - | gzip  > ../preprocess/alignment/${alignfq%.f*}_redun_${ref1%.f*}.sam.gz &&
            ...
            if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref2%.f*}.sam.gz; then
              $ngm -r ../refgenomes/$ref2 --qry ${alignfq%.f*}_uniq.fasta.gz -o ../preprocess/alignment/${alignfq%.f*}_redun.sam -R 20 -t $threads --topn 8 --strata 8 --affine &&
              awk '/@HD/ || /@SQ/{print}' ../preprocess/alignment/${alignfq%.f*}_redun.sam 2> /dev/null > ../preprocess/alignment/${alignfq%.f*}_redun_head.sam
              grep -v '^@' ../preprocess/alignment/${alignfq%.f*}_redun.sam 2> /dev/null | awk 'BEGIN{FS=OFS="\t"} !($10 == "*" && $6 !~ /^\*$/) {print}' | \
              cat ../preprocess/alignment/${alignfq%.f*}_redun_head.sam - | gzip  > ../preprocess/alignment/${alignfq%.f*}_redun_${ref2%.f*}.sam.gz &&
              rm ../preprocess/alignment/"${alignfq%.f*}"_redun.sam &&
              cp -rn ../preprocess/alignment/${alignfq%.f*}_redun_${ref2%.f*}.sam.gz ${projdir}/preprocess/alignment/ &&
              rm "${alignfq%.f*}"_uniq.fastq.gz ../preprocess/alignment/"${alignfq%.f*}"_redun_head.sam &&
              wait
            fi
            if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref3%.f*}.sam.gz; then
              $ngm -r ../refgenomes/$ref3 --qry ${alignfq%.f*}_uniq.fasta.gz -o ../preprocess/alignment/${alignfq%.f*}_redun.sam -R 20 -t $threads --topn 8 --strata 8 --affine &&
              awk '/@HD/ || /@SQ/{print}' ../preprocess/alignment/${alignfq%.f*}_redun.sam 2> /dev/null > ../preprocess/alignment/${alignfq%.f*}_redun_head.sam
              grep -v '^@' ../preprocess/alignment/${alignfq%.f*}_redun.sam 2> /dev/null | awk 'BEGIN{FS=OFS="\t"} !($10 == "*" && $6 !~ /^\*$/) {print}' | \
              cat ../preprocess/alignment/${alignfq%.f*}_redun_head.sam - | gzip  > ../preprocess/alignment/${alignfq%.f*}_redun_${ref3%.f*}.sam.gz &&
              rm ../preprocess/alignment/"${alignfq%.f*}"_redun.sam &&
      ...
      if [[ "$gap_split_align" == "true" ]]; then
        if test ! -f ${projdir}/precall_done.txt && test ! -f ${projdir}/alignment_done; then
          while IFS="" read -r alignfq || [ -n "$alignfq" ]; do
            ...
            if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref2%.f*}.sam.gz; then
              $minimap2 -t $threads -ax splice ../refgenomes/${ref2%.f*}.mmi ${alignfq%.f*}_uniq.fasta.gz > ${alignfq%.f*}_all.sam &&
              grep '^@' ${alignfq%.f*}_all.sam > ${alignfq%.f*}_header.sam &&
              grep -v '^@' ${alignfq%.f*}_all.sam | awk '$6 ~ /N/' | awk 'BEGIN{FS=OFS="\t"} !($10 == "*" && $6 !~ /^\*$/) {print}' > ${alignfq%.f*}_spliced_reads.sam &&
              grep -v '^@' ${alignfq%.f*}_all.sam | awk '$6 !~ /N/' | awk 'BEGIN{FS=OFS="\t"} !($10 == "*" && $6 !~ /^\*$/) {print}' > ${alignfq%.f*}_unspliced_reads.sam &&
              cat ${alignfq%.f*}_header.sam ${alignfq%.f*}_spliced_reads.sam > ${alignfq%.f*}_rna.sam &&
              ...
              $samtools fasta ${alignfq%.f*}_rna.bam > ${alignfq%.f*}_rna_reads.fasta &&
              $samtools fasta ${alignfq%.f*}_dna.bam > ${alignfq%.f*}_dna_reads.fasta &&
              $minimap2 -t $threads -N 8 -ax sr ../refgenomes/${ref2%.f*}.mmi ${alignfq%.f*}_dna_reads.fasta | $samtools sort -o ${alignfq%.f*}_dna_final.bam &&
              $samtools index ${alignfq%.f*}_dna_final.bam &&
              $minimap2 -t $threads -N 8 -ax splice -uf -k14 ../refgenomes/${ref2%.f*}.mmi ${alignfq%.f*}_rna_reads.fasta | $samtools sort -o ${alignfq%.f*}_rna_final.bam &&
              $samtools index ${alignfq%.f*}_rna_final.bam &&
              $GATK SplitNCigarReads -R ../refgenomes/$ref2 -I ${alignfq%.f*}_rna_final.bam -O ${alignfq%.f*}_rna_final_spliced.bam &&
              $samtools merge ${alignfq%.f*}_redun.bam ${alignfq%.f*}_dna_final.bam ${alignfq%.f*}_rna_final_spliced.bam &&
              $samtools sort -o ${alignfq%.f*}_sorted.bam ${alignfq%.f*}_redun.bam &&
              $samtools view -h ${alignfq%.f*}_sorted.bam > ../preprocess/alignment/${alignfq%.f*}_redun.sam &&
              rm ${alignfq%.f*}_*sam ${alignfq%.f*}_*bam ${alignfq%.f*}_*bai ${alignfq%.f*}_rna_reads* ${alignfq%.f*}_dna_reads* &&
            ...
            if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref3%.f*}.sam.gz; then
              $minimap2 -t $threads -ax splice ../refgenomes/${ref3%.f*}.mmi ${alignfq%.f*}_uniq.fasta.gz > ${alignfq%.f*}_all.sam &&
              grep '^@' ${alignfq%.f*}_all.sam > ${alignfq%.f*}_header.sam &&
              grep -v '^@' ${alignfq%.f*}_all.sam | awk '$6 ~ /N/' | awk 'BEGIN{FS=OFS="\t"} !($10 == "*" && $6 !~ /^\*$/) {print}' > ${alignfq%.f*}_spliced_reads.sam &&
              grep -v '^@' ${alignfq%.f*}_all.sam | awk '$6 !~ /N/' | awk 'BEGIN{FS=OFS="\t"} !($10 == "*" && $6 !~ /^\*$/) {print}' > ${alignfq%.f*}_unspliced_reads.sam &&
              cat ${alignfq%.f*}_header.sam ${alignfq%.f*}_spliced_reads.sam > ${alignfq%.f*}_rna.sam &&
              ...
              $samtools fasta ${alignfq%.f*}_rna.bam > ${alignfq%.f*}_rna_reads.fasta &&
              $samtools fasta ${alignfq%.f*}_dna.bam > ${alignfq%.f*}_dna_reads.fasta &&
              $minimap2 -t $threads -N 8 -ax sr ../refgenomes/${ref3%.f*}.mmi ${alignfq%.f*}_dna_reads.fasta | $samtools sort -o ${alignfq%.f*}_dna_final.bam &&
              $samtools index ${alignfq%.f*}_dna_final.bam &&
              $minimap2 -t $threads -N 8 -ax splice -uf -k14 ../refgenomes/${ref3%.f*}.mmi ${alignfq%.f*}_rna_reads.fasta | $samtools sort -o ${alignfq%.f*}_rna_final.bam &&
              $samtools index ${alignfq%.f*}_rna_final.bam &&
              $GATK SplitNCigarReads -R ../refgenomes/$ref3 -I ${alignfq%.f*}_rna_final.bam -O ${alignfq%.f*}_rna_final_spliced.bam &&
              $samtools merge ${alignfq%.f*}_redun.bam ${alignfq%.f*}_dna_final.bam ${alignfq%.f*}_rna_final_spliced.bam &&
              $samtools sort -o ${alignfq%.f*}_sorted.bam ${alignfq%.f*}_redun.bam &&
              $samtools view -h ${alignfq%.f*}_sorted.bam > ../preprocess/alignment/${alignfq%.f*}_redun.sam &&
              rm ${alignfq%.f*}_*sam ${alignfq%.f*}_*bam ${alignfq%.f*}_*bai ${alignfq%.f*}_rna_reads* ${alignfq%.f*}_dna_reads* &&
      ...
      if test ! -f ${projdir}/precall_done.txt && test ! -f ${projdir}/alignment_done; then
        while IFS="" read -r alignfq || [ -n "$alignfq" ]; do
          ...
          if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref1%.f*}.sam.gz; then
            ...
            $star --runThreadN $threads \
              --genomeDir ${projdir}/refgenomes/star_index_${ref1%.f*} \
            ...
            $samtools view -h ../preprocess/alignment/${alignfq%.f*}_redunsplit.bam | gzip > ../preprocess/alignment/${alignfq%.f*}_redun_${ref1%.f*}.sam.gz &&
          ...
          if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref2%.f*}.sam.gz; then
            $star --runThreadN $threads \
              --genomeDir ${projdir}/refgenomes/star_index_${ref2%.f*} \
              --readFilesIn ${alignfq%.f*}_uniq.fastq.gz \
              --readFilesCommand zcat \
              --outFileNamePrefix ../preprocess/staralign/${alignfq%.f*}_redun_ \
              --outSAMtype SAM \
            ...
            cat ../preprocess/alignment/${alignfq%.f*}_redun_head.sam -  > ../preprocess/alignment/${alignfq%.f*}_redun.sam &&
            $samtools view -S -b ../preprocess/alignment/${alignfq%.f*}_redun.sam > ../preprocess/alignment/${alignfq%.f*}_redun.bam &&
            $GATK SplitNCigarReads -R ${projdir}/refgenomes/$ref2 -I ../preprocess/alignment/${alignfq%.f*}_redun.bam -O ../preprocess/alignment/${alignfq%.f*}_redunsplit.bam &&
            $samtools view -h ../preprocess/alignment/${alignfq%.f*}_redunsplit.bam | gzip > ../preprocess/alignment/${alignfq%.f*}_redun_${ref2%.f*}.sam.gz &&
            rm ../preprocess/alignment/${alignfq%.f*}_redun.sam ../preprocess/alignment/${alignfq%.f*}_redun.bam ../preprocess/alignment/${alignfq%.f*}_redunsplit* &&
          ...
          if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun_${ref3%.f*}.sam.gz; then
            $star --runThreadN $threads \
              --genomeDir ${projdir}/refgenomes/star_index_${ref3%.f*} \
              --readFilesIn ${alignfq%.f*}_uniq.fastq.gz \
              --readFilesCommand zcat \
              --outFileNamePrefix ../preprocess/staralign/${alignfq%.f*}_redun_ \
              --outSAMtype SAM \
            ...
            cat ../preprocess/alignment/${alignfq%.f*}_redun_head.sam -  > ../preprocess/alignment/${alignfq%.f*}_redun.sam &&
            $samtools view -S -b ../preprocess/alignment/${alignfq%.f*}_redun.sam > ../preprocess/alignment/${alignfq%.f*}_redun.bam &&
            $GATK SplitNCigarReads -R ${projdir}/refgenomes/$ref3 -I ../preprocess/alignment/${alignfq%.f*}_redun.bam -O ../preprocess/alignment/${alignfq%.f*}_redunsplit.bam &&
            $samtools view -h ../preprocess/alignment/${alignfq%.f*}_redunsplit.bam | gzip > ../preprocess/alignment/${alignfq%.f*}_redun_${ref3%.f*}.sam.gz &&
            rm ../preprocess/alignment/${alignfq%.f*}_redun.sam ../preprocess/alignment/${alignfq%.f*}_redun.bam ../preprocess/alignment/${alignfq%.f*}_redunsplit* &&
    ...
    split -l 10000 ${i%.f*}_full_${ref1%.f*}.sam ${i%.f*}_${ref1%.f*}_chunk_ && rm ${i%.f*}_full_${ref1%.f*}.sam &&
    find . -name '${i%.f*}_${ref1%.f*}_chunk_*' -print0 | xargs -0 -P "$prepthreads" -I{} bash -c 'awk '\''{for(i=0;i<=$2-1;i++) print $0}'\'' "$1" > "$1.out"' _ {} &&
    cat ${i%.f*}_${ref1%.f*}_chunk_*.out | awk '!($2="")1' | awk '{$1=$1"_"NR}1' | awk '{gsub(/ /,"\t");}1' > ${i%.f*}_full_${ref1%.f*}.sam &&
    cat <(zcat ./alignment/${i%.f*}_redun_${ref1%.f*}.sam.gz | grep '^@') ${i%.f*}_full_${ref1%.f*}.sam | gzip > ${i%.f*}_full_${ref1%.f*}.sam.gz &&
    rm ${i%.f*}_${ref1%.f*}_chunk_* ${i%.f*}_full_${ref1%.f*}.sam &&
    ...
    split -l 10000 ${i%.f*}_full_${ref2%.f*}.sam ${i%.f*}_${ref2%.f*}_chunk_ && rm ${i%.f*}_full_${ref2%.f*}.sam &&
    find . -name '${i%.f*}_${ref2%.f*}_chunk_*' -print0 | xargs -0 -P "$prepthreads" -I{} bash -c 'awk '\''{for(i=0;i<=$2-1;i++) print $0}'\'' "$1" > "$1.out"' _ {} &&
    cat ${i%.f*}_${ref2%.f*}_chunk_*.out | awk '!($2="")1' | awk '{$1=$1"_"NR}1' | awk '{gsub(/ /,"\t");}1' > ${i%.f*}_full_${ref2%.f*}.sam &&
    cat <(zcat ./alignment/${i%.f*}_redun_${ref2%.f*}.sam.gz | grep '^@') ${i%.f*}_full_${ref2%.f*}.sam | gzip > ${i%.f*}_full_${ref2%.f*}.sam.gz &&
    rm ${i%.f*}_${ref2%.f*}_chunk_* ${i%.f*}_full_${ref2%.f*}.sam &&
    ...
    split -l 10000 ${i%.f*}_full_${ref3%.f*}.sam ${i%.f*}_${ref3%.f*}_chunk_ && rm ${i%.f*}_full_${ref3%.f*}.sam &&
    find . -name '${i%.f*}_${ref3%.f*}_chunk_*' -print0 | xargs -0 -P "$prepthreads" -I{} bash -c 'awk '\''{for(i=0;i<=$2-1;i++) print $0}'\'' "$1" > "$1.out"' _ {} &&
    cat ${i%.f*}_${ref3%.f*}_chunk_*.out | awk '!($2="")1' | awk '{$1=$1"_"NR}1' | awk '{gsub(/ /,"\t");}1' > ${i%.f*}_full_${ref3%.f*}.sam &&
    cat <(zcat ./alignment/${i%.f*}_redun_${ref3%.f*}.sam.gz | grep '^@') ${i%.f*}_full_${ref3%.f*}.sam | gzip > ${i%.f*}_full_${ref3%.f*}.sam.gz &&
    rm ${i%.f*}_${ref3%.f*}_chunk_* ${i%.f*}_full_${ref3%.f*}.sam &&
      ...
      if test ! -f ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_precall.bam.bai && test ! -f ${projdir}/preprocess/${i%.f*}_${ref2%.f*}_precall.bam.bai && test ! -f ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_precall.bam.bai; then
...

        {
          ...
          if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == true ]]; then
            ...
            awk -F '\t' 'BEGIN{OFS="\t"} {if ($5 == 0) {print $0}}' 2> /dev/null > ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_uniqeq.sam
            $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
            awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
          ...
          if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == false ]]; then
            ...
            awk '$1==1 || $1>6{print $0}' | awk '{print $2}' > ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_uniqsingle.sam &&
            $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
            awk -F '\t' 'NR==FNR{a[$0];next} !($1 in a)' ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_uniqsingle.sam - | \
            awk '{gsub(/_pe-/,"_pe-\t",$1);}1' | awk '{print $2"\t"$0}' | awk '{$2=$3=""}1' | tr -s " " | tr " " "\t" | \
            awk -v min=$minmapq -F '\t' 'BEGIN{OFS="\t"} {if ($5 >= min) {print $0}}' > ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_uniq.sam &&
        ...
        if [[ "${ploidy_ref1}" && "${ploidy_ref2}" ]]; then
          {
            ...
            if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == true ]]; then
              ...
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
              awk -F '\t' 'BEGIN{OFS="\t"} {if ($5 == 0) {print $0}}' 2> /dev/null > ${i%.f*}_${ref1%.f*}_${ref2%.f*}_uniqeq.sam
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
            ...
            if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == false ]]; then
              ...
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | \
              awk '$1==1 || $1>6{print $0}' | awk '{print $2}' > ${i%.f*}_${ref1%.f*}_${ref2%.f*}_uniqsingle.sam &&
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk -F '\t' 'NR==FNR{a[$0];next} !($1 in a)' ${i%.f*}_${ref1%.f*}_${ref2%.f*}_uniqsingle.sam - | \
              awk '{gsub(/_pe-/,"_pe-\t",$1);}1' | awk '{print $2"\t"$0}' | awk '{$2=$3=""}1' | tr -s " " | tr " " "\t" | \
              awk -v min=$minmapq -F '\t' 'BEGIN{OFS="\t"} {if ($5 >= min) {print $0}}' > ${i%.f*}_${ref1%.f*}_${ref2%.f*}_uniq.sam &&
        ...
        if [[ "${ploidy_ref1}" && "${ploidy_ref3}" ]]; then
          {
            ...
            if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == true ]]; then
              ...
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
              awk -F '\t' 'BEGIN{OFS="\t"} {if ($5 == 0) {print $0}}' 2> /dev/null > ${i%.f*}_${ref1%.f*}_${ref3%.f*}_uniqeq.sam
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
            ...
            if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == false ]]; then
              ...
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | \
              awk '$1==1 || $1>6{print $0}' | awk '{print $2}' > ${i%.f*}_${ref1%.f*}_${ref3%.f*}_uniqsingle.sam &&
              $samtools view -F4 ${i%.f*}_del_${ref1%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk -F '\t' 'NR==FNR{a[$0];next} !($1 in a)' ${i%.f*}_${ref1%.f*}_${ref3%.f*}_uniqsingle.sam - | \
              awk '{gsub(/_pe-/,"_pe-\t",$1);}1' | awk '{print $2"\t"$0}' | awk '{$2=$3=""}1' | tr -s " " | tr " " "\t" | \
              awk -v min=$minmapq -F '\t' 'BEGIN{OFS="\t"} {if ($5 >= min) {print $0}}' > ${i%.f*}_${ref1%.f*}_${ref3%.f*}_uniq.sam &&
        ...
        if [[ "${ploidy_ref2}" && "${ploidy_ref3}" ]]; then
          {
            ...
            if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == true ]]; then
              ...
              $samtools view -F4 ${i%.f*}_del_${ref2%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
              awk -F '\t' 'BEGIN{OFS="\t"} {if ($5 == 0) {print $0}}' 2> /dev/null > ${i%.f*}_${ref2%.f*}_${ref3%.f*}_uniqeq.sam
              $samtools view -F4 ${i%.f*}_del_${ref2%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
            ...
            if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == false ]]; then
              ...
              $samtools view -F4 ${i%.f*}_del_${ref2%.f*}_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
              awk -F '\t' 'NR==FNR{a[$0];next} !($1 in a)' ${i%.f*}_${ref2%.f*}_${ref3%.f*}_uniqsingle.sam - | \
              awk '{gsub(/_pe-/,"_pe-\t",$1);}1' | awk '{print $2"\t"$0}' | awk '{$2=$3=""}1' | tr -s " " | tr " " "\t" | \
              awk -v min=$minmapq -F '\t' 'BEGIN{OFS="\t"} {if ($5 >= min) {print $0}}' > ${i%.f*}_${ref2%.f*}_${ref3%.f*}_uniq.sam &&
        ...
        if [[ "${ploidy_ref1}" ]]; then
          {
            if [[ "$ploidy_ref1" ]]; then
              ...
              if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == true ]]; then
                ...
                $samtools view -F4 ${i%.f*}_del_${ref1%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
                awk -F '\t' 'BEGIN{OFS="\t"} {if ($5 == 0) {print $0}}' 2> /dev/null > ${i%.f*}_${ref1%.f*}_uniqeq.sam
                $samtools view -F4 ${i%.f*}_del_${ref1%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
              ...
              if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == false ]]; then
                ...
                $samtools view -F4 ${i%.f*}_del_${ref1%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | \
                awk '$1==1 || $1>6{print $0}' | awk '{print $2}' > ${i%.f*}_${ref1%.f*}_uniqsingle.sam &&
                $samtools view -F4 ${i%.f*}_del_${ref1%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk -F '\t' 'NR==FNR{a[$0];next} !($1 in a)' ${i%.f*}_${ref1%.f*}_uniqsingle.sam - | \
                awk '{gsub(/_pe-/,"_pe-\t",$1);}1' | awk '{print $2"\t"$0}' | awk '{$2=$3=""}1' | tr -s " " | tr " " "\t" | \
                awk -v min=$minmapq -F '\t' 'BEGIN{OFS="\t"} {if ($5 >= min) {print $0}}' > ${i%.f*}_${ref1%.f*}_uniq.sam &&
        ...
        if [[ "${ploidy_ref2}" ]]; then
          {
            if [[ "$ploidy_ref2" ]]; then
              ...
              if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == true ]]; then
                ...
                $samtools view -F4 ${i%.f*}_del_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
                awk -F '\t' 'BEGIN{OFS="\t"} {if ($5 == 0) {print $0}}' 2> /dev/null > ${i%.f*}_${ref2%.f*}_uniqeq.sam
                $samtools view -F4 ${i%.f*}_del_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
              ...
              if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == false ]]; then
                ...
                $samtools view -F4 ${i%.f*}_del_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | \
                awk '$1==1 || $1>6{print $0}' | awk '{print $2}' > ${i%.f*}_${ref2%.f*}_uniqsingle.sam &&
                $samtools view -F4 ${i%.f*}_del_${ref2%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk -F '\t' 'NR==FNR{a[$0];next} !($1 in a)' ${i%.f*}_${ref2%.f*}_uniqsingle.sam - | \
                awk '{gsub(/_pe-/,"_pe-\t",$1);}1' | awk '{print $2"\t"$0}' | awk '{$2=$3=""}1' | tr -s " " | tr " " "\t" | \
                awk -v min=$minmapq -F '\t' 'BEGIN{OFS="\t"} {if ($5 >= min) {print $0}}' > ${i%.f*}_${ref2%.f*}_uniq.sam &&
        ...
        if [[ "${ploidy_ref3}" ]]; then
          {
            if [[ "$ploidy_ref3" ]]; then
              ...
              if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == true ]]; then
                ...
                $samtools view -F4 ${i%.f*}_del_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
                awk -F '\t' 'BEGIN{OFS="\t"} {if ($5 == 0) {print $0}}' 2> /dev/null > ${i%.f*}_${ref3%.f*}_uniqeq.sam
                $samtools view -F4 ${i%.f*}_del_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | awk '$1==1 || $1<=6{print $0}' | awk '{$1=""}1' | awk '$1=$1' | tr " " "\t" | \
              ...
              if [[ "$paralogs" == true ]] && [[ "$uniquely_mapped" == false ]]; then
                ...
                $samtools view -F4 ${i%.f*}_del_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk '!h[$1] { g[$1]=$0 } { h[$1]++ } END { for(k in g) print h[k], g[k] }' | tr " " "\t" | tr '\r' '\n' | \
                awk '$1==1 || $1>6{print $0}' | awk '{print $2}' > ${i%.f*}_${ref3%.f*}_uniqsingle.sam &&
                $samtools view -F4 ${i%.f*}_del_${ref3%.f*}.sam | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
                awk -F '\t' 'NR==FNR{a[$0];next} !($1 in a)' ${i%.f*}_${ref3%.f*}_uniqsingle.sam - | \
                awk '{gsub(/_pe-/,"_pe-\t",$1);}1' | awk '{print $2"\t"$0}' | awk '{$2=$3=""}1' | tr -s " " | tr " " "\t" | \
                awk -v min=$minmapq -F '\t' 'BEGIN{OFS="\t"} {if ($5 >= min) {print $0}}' > ${i%.f*}_${ref3%.f*}_uniq.sam &&
  ...
  main () {
    ...
    if [[ $nodes -gt 1 ]]; then
      ...
      while IFS="" read -r i || [ -n "$i" ]; do
        sleep $((RANDOM % 2))
        cp -rn ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam* /tmp/${samples_list%.txt}/preprocess/
        cp -rn ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_precall.bam* /tmp/${samples_list%.txt}/preprocess/
        cp -rn ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref3%.f*}_precall.bam* /tmp/${samples_list%.txt}/preprocess/
        cp -rn ${projdir}/preprocess/${i%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam* /tmp/${samples_list%.txt}/preprocess/
        ...
        cp -rn ${projdir}/preprocess/${i%.f*}_${ref3%.f*}_precall.bam* /tmp/${samples_list%.txt}/preprocess/
        wait
      done < <(cat ${projdir}/${samples_list})
    fi
    ...
    if [[ ! -s $interval_list ]]; then export interval_list=""; fi
  	...
  	if [[ "$samples_list" == "samples_list_node_1.txt" ]]; then
        ...
        if [[ "$ploidy_ref1" && "$ploidy_ref2" ]]; then
          ...
          Get2_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ${projdir}/refgenomes/${ref1%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref1%.f*} '$0 ~ pat' )
          ...
          if [[ -z "$Get_Chromosome" ]]; then
            ...
            if [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ploidy}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
              ...
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref1 -L ${projdir}/refgenomes/${ref1%.f*}.list ${input}-ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
              cd ../snpcall
        ...
        if [[ "$ploidy_ref1" && "$ploidy_ref3" ]]; then
          ...
          j=-I; input=""; k=""
          for i in *_${ref1%.f*}_${ref3%.f*}_precall.bam; do
            k="${j} ${i}"; input="${input} ${k}"
          done
          Get2_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ${projdir}/refgenomes/${ref1%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref1%.f*} '$0 ~ pat' )
          if [[ -n "$Exclude_Chromosome" ]]; then
          ...
          if [[ -z "$Get_Chromosome" ]]; then
            if [[ -z "$interval_list" ]]; then
              for selchr in $Get2_Chromosome; do (
                if [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref1 -L ${selchr} ${input} -ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
                  gunzip ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz &&
                  wait
                fi
                ) &
                if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
              ...
              for selchr in $Get2_Chromosome; do (
                cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
                if [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref1 -L ${projdir}/variant_intervals_${selchr}.list ${input} -ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
                  gunzip ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz &&
                  rm ${projdir}/variant_intervals_${selchr}.list &&
                  wait
                fi
            ...
            wait
            cd ../snpcall
            grep -h '^#' ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf | awk '!visited[$0]++' | awk '!/^##GATKCommandLine/' > vcf_header.txt &&
            cat ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf | awk '!/^#/' > all.vcf &&
            cat vcf_header.txt all.vcf > ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf &&
            rm vcf_header.txt all.vcf *.vcf.gz.tb* ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf 2> /dev/null &&
            wait
          else
            if [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
              echo $Get_Chromosome | tr ',' '\n' | awk '{print "SN:"$1}' | awk 'NR==FNR{a[$1];next}$2 in a{print $0}' - ${projdir}/refgenomes/${ref1%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $2":1-"$3}' > ${projdir}/refgenomes/${ref1%.f*}.list &&
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref1 -L ${projdir}/refgenomes/${ref1%.f*}.list ${input}-ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
              cd ../snpcall
              gunzip ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf &&
              wait
            fi
          fi
        fi
        ...
        if [[ "$ploidy_ref2" && "$ploidy_ref3" ]]; then
          ...
          j=-I; input=""; k=""
          for i in *_${ref2%.f*}_${ref3%.f*}_precall.bam; do
            k="${j} ${i}"; input="${input} ${k}"
          done
          Get2_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ${projdir}/refgenomes/${ref2%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref2%.f*} '$0 ~ pat' )
          if [[ -n "$Exclude_Chromosome" ]]; then
            for i in $(echo "$Exclude_Chromosome" | tr ',' '\n'); do
              Get2_Chromosome=$(echo $Get2_Chromosome | awk -v i=$i '{gsub(i,"");}1')
            done
          ...
          if [[ -z "$Get_Chromosome" ]]; then
            if [[ -z "$interval_list" ]]; then
              for selchr in $Get2_Chromosome; do (
                if [[ "$(ls ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref2 -L ${selchr} ${input} -ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
                  gunzip ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz &&
                  wait
                fi
                ) &
                if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
              ...
              for selchr in $Get2_Chromosome; do (
                cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
                if [[ "$(ls ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref2 -L ${projdir}/variant_intervals_${selchr}.list ${input} -ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
                  gunzip ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz &&
                  rm ${projdir}/variant_intervals_${selchr}.list &&
                  wait
                fi
            ...
            wait
            cd ../snpcall
            grep -h '^#' ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf | awk '!visited[$0]++' | awk '!/^##GATKCommandLine/' > vcf_header.txt &&
            cat ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf | awk '!/^#/' > all.vcf &&
            cat vcf_header.txt all.vcf > ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf &&
            rm vcf_header.txt all.vcf *.vcf.gz.tb* ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf 2> /dev/null &&
            wait
          else
            if [[ "$(ls ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
              echo $Get_Chromosome | tr ',' '\n' | awk '{print "SN:"$1}' | awk 'NR==FNR{a[$1];next}$2 in a{print $0}' - ${projdir}/refgenomes/${ref2%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $2":1-"$3}' > ${projdir}/refgenomes/${ref2%.f*}.list &&
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref2 -L ${projdir}/refgenomes/${ref2%.f*}.list ${input}-ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
              cd ../snpcall
              gunzip ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf &&
              wait
            fi
          fi
        fi
          ...
          echo -e "${magenta}- performing SNP calling on subgenome-1 ${white}\n"
            ...
            wait
    			    ...
    			    $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref1 -L ${projdir}/refgenomes/${ref1%.f*}.list ${input}-ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref1%.f*}_${ploidy_ref1}x_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref1 * maxHaplotype)) &&
    			    cd ../snpcall
          ...
          echo -e "${magenta}- performing SNP calling on subgenome-2 ${white}\n"
            ...
            wait
    			    ...
    			    $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref2 -L ${projdir}/refgenomes/${ref2%.f*}.list ${input}-ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref2%.f*}_${ploidy_ref2}x_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref2 * maxHaplotype)) &&
    			    cd ../snpcall
        ...
        if [[ "$ploidy_ref3" ]]; then
          echo -e "${magenta}- performing SNP calling on subgenome-2 ${white}\n"
          j=-I; input=""; k=""
          for i in *_${ref3%.f*}_precall.bam; do
            k="${j} ${i}"; input="${input} ${k}"
          done
          Get2_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ${projdir}/refgenomes/${ref3%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref3%.f*} '$0 ~ pat' )
          if [[ -n "$Exclude_Chromosome" ]]; then
            for i in $(echo "$Exclude_Chromosome" | tr ',' '\n'); do
              Get2_Chromosome=$(echo $Get2_Chromosome | awk -v i=$i '{gsub(i,"");}1')
            done
          ...
          if [[ -z "$Get_Chromosome" ]]; then
            if [[ -z "$interval_list_ref3" ]]; then
              for selchr in $Get2_Chromosome; do (
                if [[ "$(ls ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R "${projdir}/refgenomes/$ref3" -L ${selchr} ${input} -ploidy $ploidy_ref3 -O ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref3 * maxHaplotype)) &&
              ...
              for selchr in $Get2_Chromosome; do (
                ...
                if [[ "$(ls ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R "${projdir}/refgenomes/$ref3" -L ${projdir}/variant_intervals_${selchr}.list ${input} -ploidy $ploidy_ref3 -O ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref3 * maxHaplotype)) &&
                  gunzip ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf.gz &&
                  rm ${projdir}/variant_intervals_${selchr}.list &&
                  wait
                fi
            ...
            rm vcf_header.txt all.vcf *.vcf.gz.tb* ${pop}_${ref3%.f*}_${ploidy_ref3}x_*_raw.vcf 2> /dev/null &&
            wait
          else
            if [[ "$(ls ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
              echo $Get_Chromosome | tr ',' '\n' | awk -v pat=${ref3%.f*} '$0 ~ pat' | awk '{print "SN:"$1}' | awk 'NR==FNR{a[$1];next}$2 in a{print $0}' - ${projdir}/refgenomes/${ref3%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $2":1-"$3}' > ${projdir}/refgenomes/${ref3%.f*}.list &&
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ${projdir}/refgenomes/$ref3 -L ${projdir}/refgenomes/${ref3%.f*}.list ${input}-ploidy $ploidy -O ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_raw.vcf.gz --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref3 * maxHaplotype)) &&
              cd ../snpcall
      ...
      if [[ ! -d "${projdir}"/snpcall/cohorts_1 ]]; then
        while IFS="" read -r i || [ -n "$i" ]; do (
          sleep $((RANDOM % 2))
          if [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
            if test ! -f "${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf"; then
              if [[ -z "$Get_Chromosome" ]]; then
                if [[ -z "$interval_list" ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip  --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
                  wait
                else
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -L ${projdir}/${interval_list} -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
                  wait
                fi
              else
                ...
                $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R "../refgenomes/$ref1" -L ../refgenomes/${ref1%.f*}.list -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy * maxHaplotype)) &&
                wait
              fi
              mv ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf.gz &&
              rm ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz.tb* 2> /dev/null &&
              gunzip ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf.gz &&
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" IndexFeatureFile -I ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf &&
              wait
            fi
          fi
          mv ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
          rm  ${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam* 2> /dev/null
          wait ) &
          if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
            wait
          fi
      ...
      cd ${projdir}/preprocess/
      printf "variant calling completed on ${samples_list}" > "${projdir}/call123_${samples_list}" &&
      wait

      if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
        ...
        if [[ $call123 == $nodes ]]; then
          ...
          if [[ ! -d cohorts_1 ]]; then
            mkdir -p cohorts_1
            mv *_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf* ./cohorts_1/
          fi
          wait
          ...
          for dir in cohorts*/; do
            cd $dir
            j=--variant; input=""; k=""
            for i in *_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf; do
              k="${j} ${i}"; input="${input} ${k}"
            done
            if [[ -z "$Get_Chromosome" ]]; then
              Get2_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ${projdir}/refgenomes/${ref1%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref1%.f*} '$0 ~ pat' )
            ...
            for selchr in $Get2_Chromosome; do (
              ...
              if [[ ! -f "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz.tbi" ]]; then
                if [[ -z "$interval_list" ]]; then
                  $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref1 -L ${selchr} -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.hold.vcf.gz && \
                  ...
                  cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
                  $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref1 -L ${projdir}/variant_intervals_${selchr}.list -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.hold.vcf.gz && \
                  rm ${projdir}/variant_intervals_${selchr}.list &&
                  rm -r ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw && \
                  mv "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.hold.vcf.gz" "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz"
                  mv "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.hold.vcf.gz.tbi" "${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw.vcf.gz.tbi" &&
            ...
            wait
            for g in ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf.gz; do (
              gunzip $g ) &
              if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
                wait
              fi
            ...
            rm ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_*_raw.vcf.gz.tb* 2> /dev/null &&
      ...
      if [[ "$ploidy_ref1" && "$ploidy_ref2" ]]; then
        ...
        if [[ ! -d "${projdir}"/snpcall/cohorts_1 ]]; then
          while IFS="" read -r i || [ -n "$i" ]; do (
            ...
            if test ! -f "${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}.g.vcf"; then
              if [[ -z "$Get_Chromosome" ]]; then
                if [[ -z "$interval_list" ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip  --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                  wait
                else
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -L ${projdir}/${interval_list} -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                  wait
                fi
              else
                echo $Get_Chromosome | tr ',' '\n' | awk '{print "SN:"$1}' | awk 'NR==FNR{a[$1];next}$2 in a{print $0}' - ../refgenomes/${ref1%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $2":1-"$3}' > ../refgenomes/${ref1%.f*}.list
                $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R "../refgenomes/$ref1" -L ../refgenomes/${ref1%.f*}.list -I ${i%.f*}_${ref1%.f*}_${ref2%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref2%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                wait
              fi
            ...
            mv ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
        ...
        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call12 == $nodes ]]; then
            ...
            for dir in cohorts*/; do
              ...
              for selchr in $Get2_Chromosome; do (
                ...
                if [[ ! -f "${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz.tbi" ]]; then
                  if [[ -z "$interval_list" ]]; then
                    $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref1 -L ${selchr} -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz && \
                    ...
                    cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
                    $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref1 -L ${projdir}/variant_intervals_${selchr}.list -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz && \
                    rm ${projdir}/variant_intervals_${selchr}.list &&
                    rm -r ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw && \
                    mv "${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz" "${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz"
                    mv "${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz.tbi" "${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz.tbi" &&
              ...
              wait
              for g in ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_*_raw.vcf.gz; do (
                gunzip $g ) &
                if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
                  wait
                fi
              ...
              rm vcf_header.txt all.vcf &&
              rm ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_*_raw.vcf ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_*_raw.vcf.gz.tb* 2> /dev/null &&
        ...
        printf "variant calling for subgenome 1 and 2 overlapped skipped" > "${projdir}/call12_${samples_list}"
      fi
      wait
      ...
      if [[ "$ploidy_ref1" && "$ploidy_ref3" ]]; then
        ...
        if [[ ! -d "${projdir}"/snpcall/cohorts_1 ]]; then
          while IFS="" read -r i || [ -n "$i" ]; do (
            sleep $((RANDOM % 2))
            if test ! -f "${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.g.vcf"; then
              if [[ -z "$Get_Chromosome" ]]; then
                if [[ -z "$interval_list" ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -I ${i%.f*}_${ref1%.f*}_${ref3%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip  --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                  wait
                else
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -L ${projdir}/${interval_list} -I ${i%.f*}_${ref1%.f*}_${ref3%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                  wait
                fi
              else
                echo $Get_Chromosome | tr ',' '\n' | awk '{print "SN:"$1}' | awk 'NR==FNR{a[$1];next}$2 in a{print $0}' - ../refgenomes/${ref1%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $2":1-"$3}' > ../refgenomes/${ref1%.f*}.list
                $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R "../refgenomes/$ref1" -L ../refgenomes/${ref1%.f*}.list -I ${i%.f*}_${ref1%.f*}_${ref3%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                wait
              fi
              mv ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.hold.g.vcf.gz ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.g.vcf.gz &&
              rm ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.hold.g.vcf.gz.tb* 2> /dev/null &&
              gunzip ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.g.vcf.gz &&
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" IndexFeatureFile -I ${projdir}/snpcall/${i%.f*}_${ref1%.f*}_${ref3%.f*}.g.vcf &&
              wait
            fi
            mv ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
            rm  ${i%.f*}_${ref1%.f*}_${ref3%.f*}_precall.bam* 2> /dev/null
            wait ) &
            if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
              wait
            fi
        ...
        wait
        cd ${projdir}/preprocess/
        printf "variant calling completed on ${samples_list}" > "${projdir}/call13_${samples_list}" &&
        wait

        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call13 == $nodes ]]; then
            ...
            if [[ ! -d cohorts_1 ]]; then
              mkdir -p cohorts_1
              mv *_${ref1%.f*}_${ref3%.f*}.g.vcf* ./cohorts_1/
            fi
            wait
            ...
            for dir in cohorts*/; do
              cd $dir
              j=--variant; input=""; k=""
              for i in *_${ref1%.f*}_${ref3%.f*}.g.vcf; do
                k="${j} ${i}"; input="${input} ${k}"
              done
              if [[ -z "$Get_Chromosome" ]]; then
                Get2_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ${projdir}/refgenomes/${ref1%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref1%.f*} '$0 ~ pat' )
                ...
                if [[ ! -f "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz.tbi" ]]; then
                  if [[ -f "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz" ]]; then
                    ...
                    if test ! -f "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz"; then
                      if [[ -z "$interval_list" ]]; then
                        $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref1 -L ${selchr} -V gendb://${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz && \
                        ...
                        cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
                        $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref1 -L ${projdir}/variant_intervals_${selchr}.list -V gendb://${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw -O ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz && \
                        rm ${projdir}/variant_intervals_${selchr}.list &&
                        rm -r ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw && \
                        mv "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz" "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz"
                        mv "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz.tbi" "${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz.tbi" &&
              ...
              wait
              for g in ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf.gz; do (
                gunzip $g ) &
                if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
                  wait
                fi
              ...
              rm vcf_header.txt all.vcf &&
              rm ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf.gz.tb* 2> /dev/null &&
        ...
        printf "variant calling for subgenome 1 and 3 overlapped skipped" > "${projdir}/call13_${samples_list}"
      fi
      wait
      ...
      if [[ "$ploidy_ref2" && "$ploidy_ref3" ]]; then
        ...
        if [[ ! -d "${projdir}"/snpcall/cohorts_1 ]]; then
          while IFS="" read -r i || [ -n "$i" ]; do (
            sleep $((RANDOM % 2))
            if test ! -f "${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf"; then
              if [[ -z "$Get_Chromosome" ]]; then
                if [[ -z "$interval_list" ]]; then
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref2 -I ${i%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip  --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                  wait
                else
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref2 -L ${projdir}/${interval_list} -I ${i%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                  wait
                fi
              else
                ...
                $GATK --java-options "$Xmxg -Djava.io.tmpdir=../snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R "../refgenomes/$ref2" -L ../refgenomes/${ref2%.f*}.list -I ${i%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam -ploidy $calcploidy -O ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((calcploidy * maxHaplotype)) &&
                wait
              fi
              mv ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf.gz &&
              rm ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.hold.g.vcf.gz.tb* 2> /dev/null &&
              gunzip ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf.gz &&
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" IndexFeatureFile -I ${projdir}/snpcall/${i%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf &&
              wait
            fi
            mv ${projdir}/preprocess/${i%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
            rm  ${i%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam* 2> /dev/null
            wait ) &
            if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
              wait
            fi
        ...
        wait
        cd ${projdir}/preprocess/
        printf "variant calling completed on ${samples_list}" > "${projdir}/call23_${samples_list}" &&
        wait

        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call23 == $nodes ]]; then
            ...
            if [[ ! -d cohorts_1 ]]; then
              mkdir -p cohorts_1
              mv *_${ref2%.f*}_${ref3%.f*}.g.vcf* ./cohorts_1/
            fi
            wait
            ...
            for dir in cohorts*/; do
              cd $dir
              j=--variant; input=""; k=""
              for i in *_${ref2%.f*}_${ref3%.f*}.g.vcf; do
                k="${j} ${i}"; input="${input} ${k}"
              done
              if [[ -z "$Get_Chromosome" ]]; then
                Get2_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ${projdir}/refgenomes/${ref2%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref2%.f*} '$0 ~ pat' )
              ...
              for selchr in $Get2_Chromosome; do (
                if [[ ! -f ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz.tbi ]]; then
                  ...
                  if test ! -f "${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz"; then
                    if [[ -z "$interval_list" ]]; then
                      $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref2 -L ${selchr} -V gendb://${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw -O ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz && \
                      ...
                      cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
                      $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref2 -L ${projdir}/variant_intervals_${selchr}.list -V gendb://${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw -O ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz && \
                      rm ${projdir}/variant_intervals_${selchr}.list &&
                      rm -r ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw && \
                      mv "${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz" "${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz"
                      mv "${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.hold.vcf.gz.tbi" "${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw.vcf.gz.tbi" &&
              ...
              wait
              for g in ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf.gz; do (
                gunzip $g ) &
                if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
                  wait
                fi
              ...
              rm vcf_header.txt all.vcf &&
              rm ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf.gz.tb* 2> /dev/null &&
        ...
        printf "variant calling for subgenome 2 and 3 overlapped skipped" > "${projdir}/call23_${samples_list}"
      fi
      wait
      ...
      if [[ "$ploidy_ref1" ]]; then
        ...
        printf "variant calling for subgenome 1 alone" > "${projdir}/call1_${samples_list}"
      fi
      wait
      ...
      if [[ "$ploidy_ref2" ]]; then
        ...
        if [[ ! -d "${projdir}"/snpcall/cohorts_1 ]]; then
            ...
            sleep $((RANDOM % 2))
                ...
                $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" IndexFeatureFile -I ${projdir}/snpcall/${i%.f*}_${ref2%.f*}.g.vcf &&
    				...
    				mv ${projdir}/preprocess/${i%.f*}_${ref2%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
        ...
        printf "variant calling for subgenome 2 alone" > "${projdir}/call2_${samples_list}"
      fi
      wait
      ...
      if [[ "$ploidy_ref3" ]]; then
        ...
        if [[ ! -d "${projdir}"/snpcall/cohorts_1 ]]; then
          while IFS="" read -r i || [ -n "$i" ]; do (
            sleep $((RANDOM % 2))
            if [[ "$(ls ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
              if test ! -f "${projdir}/snpcall/${i%.f*}_${ref3%.f*}.g.vcf" && test ! -f "${projdir}/snpcall/${i%.f*}_${ref3%.f*}.g.vcf.gz"; then
                if [[ -z "$Get_Chromosome" ]]; then
                  if [[ -z "$interval_list_ref3" ]]; then
                    $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref3 -I ${i%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy_ref3 -O ${projdir}/snpcall/${i%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref3 * maxHaplotype)) &&
                    wait
                  else
                    $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref3 -L ${projdir}/${interval_list_ref3} -I ${i%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy_ref3 -O ${projdir}/snpcall/${i%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref3 * maxHaplotype)) &&
                    wait
                  fi
                else
                  echo $Get_Chromosome | tr ',' '\n' | awk '{print "SN:"$1}' | awk 'NR==FNR{a[$1];next}$2 in a{print $0}' - ../refgenomes/${ref3%.f*}.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $2":1-"$3}' > ../refgenomes/${ref3%.f*}.list
                  $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref3 -L ../refgenomes/${ref3%.f*}.list -I ${i%.f*}_${ref3%.f*}_precall.bam -ploidy $ploidy_ref3 -O ${projdir}/snpcall/${i%.f*}_${ref3%.f*}.hold.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --minimum-mapping-quality $minmapq --dont-use-soft-clipped-bases $dont_use_softclip --max-num-haplotypes-in-population $((ploidy_ref3 * maxHaplotype)) &&
                  wait
                fi
                mv "${projdir}/snpcall/${i%.f*}_${ref3%.f*}.hold.g.vcf.gz" "${projdir}/snpcall/${i%.f*}_${ref3%.f*}.g.vcf.gz" &&
                rm "${projdir}/snpcall/${i%.f*}_${ref3%.f*}.hold.g.vcf.gz.tb*" 2> /dev/null &&
                gunzip ${projdir}/snpcall/${i%.f*}_${ref3%.f*}.g.vcf.gz &&
                $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" IndexFeatureFile -I ${projdir}/snpcall/${i%.f*}_${ref3%.f*}.g.vcf &&
                wait
              fi
            fi
            mv ${projdir}/preprocess/${i%.f*}_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
            rm  ${i%.f*}_${ref3%.f*}_precall.bam* 2> /dev/null
            wait ) &
            if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
              wait
            fi
        ...
        wait
        cd ${projdir}/preprocess/
        printf "variant calling completed on ${samples_list}" > "${projdir}/call3_${samples_list}" &&
        wait

        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call3 == $nodes ]]; then
            ...
            for dir in cohorts*/; do
              ...
              for selchr in $Get2_Chromosome; do (
                if [[ ! -f ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf.gz ]]; then
                  export TILEDB_DISABLE_FILE_LOCKING=1
                  if test ! -f "${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf.gz"; then
                    if [[ -z "$interval_list_ref3" ]]; then
                      ...
                      cat ${projdir}/${interval_list_ref3} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
                      $GATK --java-options "$Xmx1 -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$loopthreads"  GenotypeGVCFs -R ${projdir}/refgenomes/$ref3 -L ${projdir}/variant_intervals_${selchr}.list -V gendb://${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw -O ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.hold.vcf.gz && \
                      rm ${projdir}/variant_intervals_${selchr}.list &&
                      rm -r ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw && \
                      mv ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.hold.vcf.gz ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf.gz &&
                      mv ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.hold.vcf.gz.tbi ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw.vcf.gz.tbi &&
              ...
              wait
              for g in ${pop}_${ref3%.f*}_${ploidy_ref3}x_*_raw.vcf.gz; do (
                gunzip $g ) &
                if [[ $(jobs -r -p | wc -l) -ge $N ]]; then
                  wait
                fi
              ...
              cat vcf_header.txt all.vcf > ${pop}_${ref3%.f*}_${ploidy_ref3}x_raw.vcf &&
              rm vcf_header.txt all.vcf &&
              rm ${pop}_${ref3%.f*}_${ploidy_ref3}x_*_raw.vcf ${pop}_${ref3%.f*}_${ploidy_ref3}x_*_raw.vcf.gz.tb* 2> /dev/null &&
        ...
        printf "variant calling for subgenome 3 alone" > "${projdir}/call3_${samples_list}"
      fi
      wait
      ...
      if [[ "$variant_caller" == "bcftools" ]]; then
        cd ${projdir}/preprocess
        ls *_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam > bam_list.txt
        $bcftools mpileup -Ou -f ../refgenomes/$ref1 -b bam_list.txt | \
        $bcftools call -mv -Ov -o ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf &&
        rm bam_list.txt ploidy.txt && ${projdir}/snpcall/ &&
        $bcftools view -I ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf -O z -o ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
        $bcftools index ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
        mv ${projdir}/preprocess/*_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
        wait
      fi
      ...
      if [[ "$ploidy_ref1" && "$ploidy_ref2" ]]; then
        if [[ "$variant_caller" == "bcftools" ]]; then
          cd ${projdir}/preprocess
          ls *_${ref1%.f*}_${ref2%.f*}_precall.bam > bam_list.txt
          $bcftools mpileup -Ou -f ../refgenomes/$ref1 -b bam_list.txt | \
          $bcftools call -mv -Ov -o ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ploidy}x_raw.vcf &&
          rm bam_list.txt && ${projdir}/snpcall/ &&
          ...
          mv ${projdir}/preprocess/*_${ref1%.f*}_${ref2%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
          wait
        fi
      fi
      ...
      if [[ "$ploidy_ref1" && "$ploidy_ref3" ]]; then
        if [[ "$variant_caller" == "bcftools" ]]; then
          cd ${projdir}/preprocess
          ls *_${ref1%.f*}_${ref3%.f*}_precall.bam > bam_list.txt
          $bcftools mpileup -Ou -f ../refgenomes/$ref1 -b bam_list.txt | \
          $bcftools call -mv -Ov -o ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf &&
          rm bam_list.txt && ${projdir}/snpcall/ &&
          $bcftools view -I ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf -O z -o ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
          $bcftools index ${pop}_${ref1%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
          mv ${projdir}/preprocess/*_${ref1%.f*}_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
          wait
        fi
      fi
      ...
      if [[ "$ploidy_ref2" && "$ploidy_ref3" ]]; then
        if [[ "$variant_caller" == "bcftools" ]]; then
          cd ${projdir}/preprocess
          ls *_${ref2%.f*}_${ref3%.f*}_precall.bam > bam_list.txt
          $bcftools mpileup -Ou -f ../refgenomes/$ref2 -b bam_list.txt | \
          $bcftools call -mv -Ov -o ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf &&
          rm bam_list.txt && ${projdir}/snpcall/ &&
          $bcftools view -I ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf -O z -o ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
          $bcftools index ${pop}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
          mv ${projdir}/preprocess/*_${ref2%.f*}_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
          wait
        fi
      fi

      echo -e "${magenta}- performing SNP calling across entire genome (subgenome 1) ${white}\n"
      if [[ "$ploidy_ref1" ]]; then
        if [[ "$variant_caller" == "bcftools" ]]; then
          cd ${projdir}/preprocess
          ls *_${ref1%.f*}_precall.bam > bam_list.txt
          $bcftools mpileup -Ou -f ../refgenomes/$ref1 -b bam_list.txt | \
      ...
      echo -e "${magenta}- performing SNP calling across entire genome (subgenome 2) ${white}\n"
      if [[ "$ploidy_ref2" ]]; then
        if [[ "$variant_caller" == "bcftools" ]]; then
          cd ${projdir}/preprocess
          ls *_${ref2%.f*}_precall.bam > bam_list.txt
          $bcftools mpileup -Ou -f ../refgenomes/$ref2 -b bam_list.txt | \
          $bcftools call -mv -Ov -o ${projdir}/snpcall/${pop}_${ref2%.f*}_${ploidy}x_raw.vcf &&
          rm bam_list.txt ploidy.txt && ${projdir}/snpcall/ &&
          $bcftools view -I ${pop}_${ref2%.f*}_${ploidy}x_raw.vcf -O z -o ${pop}_${ref2%.f*}_${ploidy}x_raw.vcf.gz &&
          $bcftools index ${pop}_${ref2%.f*}_${ploidy}x_raw.vcf.gz &&
          mv ${projdir}/preprocess/*_${ref2%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
          wait
        fi
      fi
      echo -e "${magenta}- performing SNP calling across entire genome (subgenome 3) ${white}\n"
      if [[ "$ploidy_ref3" ]]; then
        if [[ "$variant_caller" == "bcftools" ]]; then
          cd ${projdir}/preprocess
          ls *_${ref3%.f*}_precall.bam > bam_list.txt
          $bcftools mpileup -Ou -f ../refgenomes/$ref3 -b bam_list.txt | \
          $bcftools call -mv -Ov -o ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy}x_raw.vcf &&
          rm bam_list.txt ploidy.txt && ${projdir}/snpcall/ &&
          $bcftools view -I ${pop}_${ref3%.f*}_${ploidy}x_raw.vcf -O z -o ${pop}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
          $bcftools index ${pop}_${ref3%.f*}_${ploidy}x_raw.vcf.gz &&
          mv ${projdir}/preprocess/*_${ref3%.f*}_precall.bam* ${projdir}/preprocess/processed/ 2> /dev/null
          wait
        fi
      fi
    	...
    	if [ -d "${projdir}/snpfilter/${snpfilter_dir}" ]; then
          ...
          if [[ "$filtered_vcf" == "true" ]]; then
            ...
            if test ! -f $projdir/split_done.txt; then
              ...
              gunzip $projdir/snpcall/*x.vcf.gz 2> /dev/null &&
              wait
