...
if [ "$ncohorts" == 1 ]; then
	...
	if [[ "$(wc -l ../refgenomes/panref.dict | awk '{print $1}')" -le 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$java $Xmx3 -XX:ParallelGCThreads=$gthreads -jar $GATK HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy $ploidy -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $((ploidy * paralogs)) )&
		...
		$java $Xmx3 -XX:ParallelGCThreads=$gthreads -jar $GATK HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy $ploidy -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $((ploidy * paralogs))
	fi
...
if [ "$ncohorts" != 1 ]; then
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' )
		for selchr in $Get_Chromosome; do (
			$java $Xmx3 -XX:ParallelGCThreads=$gthreads -jar $GATK GenomicsDBImport $input -L $selchr --genomicsdb-workspace-path ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_"${selchr}"_raw
			$java $Xmx3 -XX:ParallelGCThreads=$gthreads -jar $GATK GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_"${selchr}"_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_"${selchr}"_raw.vcf.gz
...
if [ "$ncohorts" != 1 ]; then
	...
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		...
		for i in $(ls -S *_${ref1%.f*}_precall.bam); do (
			$java $Xmx3 -XX:ParallelGCThreads=$gthreads -jar $GATK HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${ploidy_ref1} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population ${ploidy_ref1}
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
