      ...
      if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
        ...
        if [[ $call123 == $nodes ]]; then
          ...
          for dir in cohorts*/; do
            ...
            for selchr in $Get2_Chromosome; do (
              if [[ ! -d ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ploidy}x_${selchr}_raw ]]; then
        ...
        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call12 == $nodes ]]; then
            ...
            for dir in cohorts*/; do
              ...
              for selchr in $Get2_Chromosome; do (
                if [[ ! -d ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_${selchr}_raw ]]; then
                  export TILEDB_DISABLE_FILE_LOCKING=1
                  if [[ -z "$interval_list" ]]; then
        ...
        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call13 == $nodes ]]; then
            ...
            for dir in cohorts*/; do
              ...
              for selchr in $Get2_Chromosome; do (
                if [[ ! -d ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw ]]; then
                  export TILEDB_DISABLE_FILE_LOCKING=1
                  if [[ -z "$interval_list" ]]; then
        ...
        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call23 == $nodes ]]; then
            ...
            for dir in cohorts*/; do
              ...
              for selchr in $Get2_Chromosome; do (
                if [[ ! -d ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_${selchr}_raw ]]; then
                  export TILEDB_DISABLE_FILE_LOCKING=1
                  if [[ -z "$interval_list" ]]; then
        ...
        if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
          ...
          if [[ $call3 == $nodes ]]; then
            ...
            for dir in cohorts*/; do
              ...
              for selchr in $Get2_Chromosome; do (
                if [[ ! -d ${pop}_${ref3%.f*}_${ploidy_ref3}x_${selchr}_raw ]]; then
                  export TILEDB_DISABLE_FILE_LOCKING=1
                  if [[ -z "$interval_list_ref3" ]]; then
