        ...
        if [[ "$i" == *"_uniq.fasta"* || "$i" == *"fq.gz"* ]]; then
          :
  				...
  				if [[ $(file $i 2> /dev/null) =~ gzip ]]; then
  					fa_fq=$($pigzdc $projdir/samples/$i 2> /dev/null | head -n1 | cut -c1-1)
  				else
  				...
  				if [[ $(file $i 2> /dev/null) =~ gzip ]]; then
  					if [[ "${fa_fq}" == "@" ]]; then
              awk 'NR%4 == 1 {print} NR%4 == 2 {print}' <($pigzdc $i) | \
              ...
              mv nopolymers_${i} $i &&
  						awk 'NR%2==0' <($pigzdc $i) | awk 'NR%2==1' | awk 'BEGIN{srand();} {a[NR]=$0} END{for(i=1; i<=1000; i++){x=int(rand()*NR) + 1; print a[x];}}' > ${i}_length_distribution.txt
  					fi
  					if [[ "${fa_fq}" == ">" ]]; then
              awk '{gsub(/AAAAAAAAAA/,"\t"); gsub(/CCCCCCCCCC/,"\t"); gsub(/GGGGGGGGGG/,"\t"); gsub(/TTTTTTTTTT/,"\t"); print $1}' <($pigzdc $i) | gzip > nopolymers_${i} &&
              mv nopolymers_${i} $i &&
      ...
      for i in *.f*; do (
        if [[ "$i" == *"_uniq.fasta"* || "$i" == *"fq.gz"* ]]; then
          :
  				...
  				if [[ $(file $i 2> /dev/null) =~ gzip ]]; then
  					if [[ "${fa_fq}" == "@" ]]; then
              awk 'NR%2==0' <($pigzdc $i) | awk 'NR%2==1' | awk -v max=$max_seqread_len '{print substr($0,1,max)}' | awk 'NF' | \
              ...
              wait
  					...
  					if [[ "${fa_fq}" == ">" ]]; then
              grep -v '^>' <($pigzdc $i) | awk -v max=$max_seqread_len '{print substr($0,1,max)}' | awk 'NF' | \
        ...
        if [[ "$i" == *"_uniq.fasta"* || "$i" == *"_R1_uniq.fasta"* || "$i" == *"_R2_uniq.fasta"* || "$i" == *"_uniq.hold.fasta"* || "$i" == *"_R1_uniq.hold.fasta"* || "$i" == *"_R2_uniq.hold.fasta"* || "$i" == *"fq.gz"* ]]; then
          :
            ...
            if [[ $(file $i 2> /dev/null) =~ gzip ]]; then
              ...
              if [[ "${fa_fq}" == ">" ]]; then
                awk '{if(NR==1) {print $0} else {if($0 ~ /^>/) {print "\n"$0} else {printf $0}}}' <($pigzdc $i) | awk 'NR%2==0' | awk '{gsub(/ATGCAT/,"ATGCAT\nATGCAT");}1' | \
                awk 'length >= 64 && length <= 600' | grep '^ATGCAT.*ATGCAT$' | awk '{print ">frag"NR"\n"$0}' | $gzip > ${i%.f*}.tmp1.gz &&
                awk '{if(NR==1) {print $0} else {if($0 ~ /^>/) {print "\n"$0} else {printf $0}}}' <($pigzdc $i) | awk 'NR%2==0' | awk '{gsub(/CATG/,"CATG\nCATG");}1' | \
...
main () {
    ...
    while IFS="" read -r i || [ -n "$i" ]; do (
      ...
      if test ! -f ${projdir}/preprocess/alignment/${i%.f*}_redun.sam.gz && test ! -f ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_precall.bam.bai; then
        if test ! -f ${i%.f*}_uniq.fasta.gz; then
          $pigzdc $i | awk 'NR%2==0' | awk 'NR%2' | gzip > ${i%.f*}_R1_uniq.txt.gz 2> /dev/null &&
          wait
          if test -f ${i%.f*}_R2.fastq.gz; then
            $pigzdc ${i%.f*}_R2.fastq.gz | awk 'NR%2==0' | awk 'NR%2' | gzip > ${i%.f*}_R2_uniq.txt.gz 2> /dev/null &&
            wait
          fi
    ...
    while IFS="" read -r i || [ -n "$i" ]; do (
      ...
      if test ! -f ${projdir}/preprocess/alignment/${i%.f*}_redun.sam.gz && test ! -f ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_precall.bam.bai; then
        if test ! -f ${i%.f*}_uniq.fasta.gz; then
          cat ${i%.f*}_R1_uniq.txt.gz ${i%.f*}_R2_uniq.txt.gz | $pigzdc | awk '{!seen[$0]++}END{for (i in seen) print seen[i], i}' | awk '{$1=$1};1' | gzip > ${i%.f*}_repeatn.txt.gz &&
          repeatn=$($pigzdc ${i%.f*}_repeatn.txt.gz | awk '{print $1}' | sort -n | awk '{all[NR] = $1} END{print all[int(NR*0.999 - 0.5)]}')
          $pigzdc ${i%.f*}_repeatn.txt.gz | awk -v pat="$repeatn" '$1 < pat' | \
    ...
    if [[ "$aligner" == "minimap2" ]]; then
      if test ! -f ${projdir}/precall_done.txt && test ! -f ${projdir}/alignment_done; then
        while IFS="" read -r alignfq || [ -n "$alignfq" ]; do
          sleep $((RANDOM % 2))
          if test ! -f ../preprocess/alignment/${alignfq%.f*}_redun.sam.gz; then
            $minimap2 -x sr --secondary=no --min-occ-floor=1000 ../refgenomes/${ref1%.f*}.mmi <($pigzdc ${alignfq%.f*}_uniq.fasta.gz) | \
            awk '{for(i=1;i<=NF;i++) if($i ~ /^cm:i:/){split($i,a,":"); if(a[3] <= 10) print $1}}' | \
            awk 'NR==FNR{keep[$1]=1;next}
                   (NR%2==1){id=substr($1,2);p=keep[id]}
                   {if(p)print}' - <($pigzdc ${alignfq%.f*}_uniq.fasta.gz) | \
  ...
  while IFS="" read -r i || [ -n "$i" ]; do (
    ...
    $pigzdc ./alignment/${i%.f*}_redun.sam.gz | grep -v '^@' | awk '{gsub(/_/,"\t",$1);}1' | awk '{gsub(/se-/,"",$2);gsub(/pe-/,"",$2);}1' | tr ' ' '\t' > ${i%.f*}_full.sam &&
    ...
    cat <($pigzdc ./alignment/${i%.f*}_redun.sam.gz | grep '^@') ${i%.f*}_full.sam | gzip > ${i%.f*}_full.sam.gz &&
    ...
    $samtools view -F4 <($pigzdc ${i%.f*}_full.sam.gz 2> /dev/null) | awk '{print $1}' | awk '{gsub(/_pe-/,"\t");gsub(/seq/,"");}1' | \
    ...
    export min_hap=$minhap
      ...
      if test ! -f ${projdir}/preprocess/${i%.f*}_${ref1%.f*}_precall.bam.bai; then
        if [[ "$paralogs" == false ]] && [[ "$uniquely_mapped" == true ]]; then
          awk '/@HD/ || /@SQ/{print}' <($pigzdc ./alignment/${i%.f*}_redun.sam.gz 2> /dev/null) > ${i%.f*}_heading.sam &&
          $samtools view -F4 <($pigzdc ./alignment/${i%.f*}_redun.sam.gz 2> /dev/null) | grep -v '^@' | awk '$3 != "*"' 2> /dev/null | awk '$6 != "*"' 2> /dev/null | \
...
if [ `ls -1 *biallelic* 2>/dev/null | wc -l` -eq 0 ]; then
  for bial in *_DP_GT.txt; do
    ...
    grep -v '^#' <($pigzdc *${bialv#*_}_raw.vcf.gz) | awk '{print $1"\t"$2"\t"$5}' | grep -v ',' | awk '{print $1"\t"$2}' | \
    ...
    if [ -d "${projdir}/snpfilter/${snpfilter_dir}" ]; then
      ...
      export ploidydir=${snpfilter_dir:0:1} &&
        ...
        if [[ "$filtered_vcf" == "true" ]]; then
          ...
          wait
          $pigzdc ${i%rd*}split.vcf.gz | grep '^#' | awk -v pat1="${n}_Chr" -v pat2="${n}_chr" '{gsub(pat1,"Chr");gsub(pat2,"Chr");gsub(/chr/,"Chr");}1' > ${i%.txt}_header.vcf &&
          awk 'FNR==NR{a[$1,$2]=$0;next}{if(b=a[$2,$3]){print b}}' <($pigzdc ${i%rd*}split.vcf.gz | grep -v '^#' | awk -v pat1="${n}_Chr" -v pat2="${n}_chr" '{gsub(pat1,"Chr");gsub(pat2,"Chr");gsub(/chr/,"Chr");}1') <(awk '!seen[$0] {print} {++seen[$0]}' $i) | \
