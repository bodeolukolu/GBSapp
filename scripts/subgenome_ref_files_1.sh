...
if [[ "$paleopolyploid" != true ]] && [[ "$ncohorts" == no ]]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -L $selchr $input -ploidy $ploidy -O ../snpcall/${pop}_${ploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $ploidy )&
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$threads" HaplotypeCaller -R ../refgenomes/$ref1 $input -ploidy $ploidy -O ../snpcall/${pop}_${ploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $ploidy
...
if [[ "$paleopolyploid" = true ]] && [[ "$ncohorts" == no ]]; then
	...
	for c in {1..4}; do
		...
		if [[ "$Get_Chromosome" -gt 2000 ]]; then
			for selchr in $Get_Chromosome; do (
				$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads"  HaplotypeCaller -R ../refgenomes/$ref1 -L $selchr $input -ploidy $m -O ../snpcall/${pop}_${m}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $m )&
				if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
					wait
				fi
			done
			...
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads"  HaplotypeCaller -R ../refgenomes/$ref1 $input -ploidy $m -O ../snpcall/${pop}_${m}x_raw.vcf.gz --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $m
		fi
	done
	cd ${projdir}/preprocess
	mv *_precall* ./processed/
...
if [[ "$paleopolyploid" = false ]] && [[ "$ncohorts" != no ]]; then
	...
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		...
		for i in $(ls -S *_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -I $i -ploidy $ploidy -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $ploidy
	...
	for c in {1..4}; do
		...
		if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
			...
			for i in $(ls -S *${m}x_precall.bam); do (
				$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -I $i -ploidy $m -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --max-num-haplotypes-in-population $m
				cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
				if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
					wait
				fi
