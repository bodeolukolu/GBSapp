...
for snpfilter_dir in $(ls -d */); do
	if [ -d "$snpfilter_dir" ]; then
		...
		for i in $(ls *dose.txt 2> /dev/null); do
			...
			awk -v n="$n" '{gsub(n,""); gsub(/CHROM_POS/,"SNP");}1' > ${ARfile%.txt}_metric.txt
			grep -v 'CHROM' ${ARfile%.txt}_metric.txt | awk -F'\t' '{$1=$2=$3=$4=$5=""}1' | awk -F'\t' '{gsub(/na/,"0");}1' | \
			...
			cat <(printf "Allele_ratio_mean\n") - | paste <(awk '{print $1"\t"$2"\t"$3}' ${ARfile%.txt}_metric.txt) - > ${ARfile%.txt}_mean.txt
		done
		wait
