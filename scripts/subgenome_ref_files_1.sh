...
if ls *.dict 1> /dev/null 2>&1; then
	...
	for reffile in $(ls *.f*); do
		awk '/^>/{close("file.fasta"f);f++}{print $0 > "./split/file"f}' "$reffile"
	done
	cd split
...
if [ "$paleopolyploid" == "true" ]; then
	...
	for i in $(ls -S *.f* | grep -v R2.f); do (
		...
		grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 >= 1) && ($1 <= paralogs)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
...
if [ "$paleopolyploid" == "true" ]; then
	...
	for i in $(ls -S *.f* | grep -v R2.f); do (
		...
		awk -v scaleRD=$scaleRD '{print int($1/scaleRD),"\t",$2,"\t",$3,"\t",$4}' | awk -v maxRD=$maxRD '$1 <= maxRD' > ${i%.f*}_rdrefseq.txt
		awk 'NF==2 {print ">seq"NR"_se-"$1"\n"$2}' ${i%.f*}_rdrefseq.txt > ${i%.f*}_rdrefseq_se.txt
		awk 'NF==3 {print ">seq"NR"_pe-"$0}' ${i%.f*}_rdrefseq.txt | awk '{print $1"\n"$3}' > ${i%.f*}_uniq_R2.fasta
		awk 'NF==3 {print ">seq"NR"_pe-"$0}' ${i%.f*}_rdrefseq.txt | awk '{print $1"\n"$2}' | cat - ${i%.f*}_rdrefseq_se.txt > ${i%.f*}_uniq_R1.fasta
		nempty=$( wc -l ${i%.f*}_uniq_R2.fasta | awk '{print $1}' )
		...
		unset IFS; printf "%s\t%s\t%s\t%*s\n" $(sed 's/$/ |/' ../alignment_summaries/copy_number/${i%.f*}_plot.txt) | tr ' ' '|' | sort -k1 -n >> ../alignment_summaries/copy_number/${i%.f*}_copy_number_Unique_Read_histogram.txt && rm ../alignment_summaries/copy_number/${i%.f*}_copy_number.txt ../alignment_summaries/copy_number/${i%.f*}_plot.txt && \
		awk '{if ($2=="@HD" || $2=="@SQ" || $2=="@PG") {print}}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
		grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 == 1)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
		awk '!($1="")' ../preprocess/${i%.f*}_3del.sam | awk '{$1=$1};1' > ../preprocess/${i%.f*}_${ref1%.f*}.sam
		awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
		awk '!/^@/ { print }' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/^.*-/,"",$1); print $0}' | awk '{gsub(/^.*_/,"",$2); print $0}' > ../preprocess/${i%.f*}_uniq.sam
		for j in $(LC_ALL=C; sort -n -k1,1 ../preprocess/${i%.f*}_uniq.sam | grep -v @ | awk '{print $1}' | uniq); do
		...
		awk '{if ($2=="@HD" || $2=="@SQ" || $2=="@PG") {print}}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
		grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 == 2)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
		awk '!($1="")' ../preprocess/${i%.f*}_3del.sam | awk '{$1=$1};1' > ../preprocess/${i%.f*}_${ref1%.f*}.sam
		awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
		awk '!/^@/ { print }' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/^.*-/,"",$1); print $0}' | awk '{gsub(/^.*_/,"",$2); print $0}' > ../preprocess/${i%.f*}_uniq.sam
		for j in $(LC_ALL=C; sort -n -k1,1 ../preprocess/${i%.f*}_uniq.sam | grep -v @ | awk '{print $1}' | uniq); do
		...
		if [[ "$ploidy" -ge 6 ]]; then
			awk '{if ($2=="@HD" || $2=="@SQ" || $2=="@PG") {print}}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
			grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 == 3)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
			awk '!($1="")' ../preprocess/${i%.f*}_3del.sam | awk '{$1=$1};1' > ../preprocess/${i%.f*}_${ref1%.f*}.sam
			awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
			awk '!/^@/ { print }' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/^.*-/,"",$1); print $0}' | awk '{gsub(/^.*_/,"",$2); print $0}' > ../preprocess/${i%.f*}_uniq.sam
			for j in $(LC_ALL=C; sort -n -k1,1 ../preprocess/${i%.f*}_uniq.sam | grep -v @ | awk '{print $1}' | uniq); do
		...
		if [[ "$ploidy" -ge 8 ]]; then
			awk '{if ($2=="@HD" || $2=="@SQ" || $2=="@PG") {print}}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
			grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 == 4)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
			awk '!($1="")' ../preprocess/${i%.f*}_3del.sam | awk '{$1=$1};1' > ../preprocess/${i%.f*}_${ref1%.f*}.sam
			awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
			awk '!/^@/ { print }' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/^.*-/,"",$1); print $0}' | awk '{gsub(/^.*_/,"",$2); print $0}' > ../preprocess/${i%.f*}_uniq.sam
			for j in $(LC_ALL=C; sort -n -k1,1 ../preprocess/${i%.f*}_uniq.sam | grep -v @ | awk '{print $1}' | uniq); do
...
######################
if [[ "$paleopolyploid" != true ]] && [[ "$ncohorts" == no ]]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -L $selchr $input -ploidy $ploidy -O ../snpcall/${pop}_${ploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
		...
		wait
		cd ../snpcall
		ls ${pop}_${ploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$threads" HaplotypeCaller -R ../refgenomes/$ref1 $input -ploidy $ploidy -O ../snpcall/${pop}_${ploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
...
if [[ "$paleopolyploid" = true ]] && [[ "$ncohorts" == no ]]; then
	...
	for c in {1..4}; do
		...
		if [[ "$Get_Chromosome" -gt 2000 ]]; then
			for selchr in $Get_Chromosome; do (
				$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads"  HaplotypeCaller -R ../refgenomes/$ref1 -L $selchr $input -ploidy $m -O ../snpcall/${pop}_${m}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
				if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
					wait
				fi
			done
			wait
			cd ../snpcall
			ls ${pop}_${m}x_*_raw.vcf.gz | parallel gunzip
			...
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads"  HaplotypeCaller -R ../refgenomes/$ref1 $input -ploidy $m -O ../snpcall/${pop}_${m}x_raw.vcf.gz --max-reads-per-alignment-start 0
		fi
	done
	cd ${projdir}/preprocess
	mv *_precall* ./processed/
...
if [[ "$paleopolyploid" = false ]] && [[ "$ncohorts" != no ]]; then
	echo -e "${magenta}- performing SNP calling ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	mkdir ../snpcall/transit; cp ../snpcall/processed/*.g.vcf.gz* ../snpcall/transit
	mv ../snpcall/transit/*.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		...
		for i in $(ls -S *_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -I $i -ploidy $ploidy -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0
	...
	for dir in cohorts*/; do
		...
		cd ../
		bgzip ${pop}_${ploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
...
if [[ "$paleopolyploid" = true ]] && [[ "$ncohorts" != no ]]; then
	...
	for c in {1..4}; do
		m=$((c*2))
		mv ../snpcall/transit/*${m}x.g.vcf.gz* ../snpcall/
		if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
			echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
		else
			for i in $(ls -S *${m}x_precall.bam); do (
				$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/$ref1 -I $i -ploidy $m -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0
				cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
				if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
					wait
				fi
		...
		for dir in *cohorts*/; do
			...
			cd ../
			bgzip ${pop}_${m}x_raw_${dir%/}.vcf
			tabix -p vcf ${pop}_${m}x_raw_${dir%/}.vcf.gz
			wait
		done
		wait
		if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
