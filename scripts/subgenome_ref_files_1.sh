...
main () {
  ...
  while IFS="" read -r i || [ -n "$i" ]; do (
    ...
    grep -v '^@' <(zcat ./alignment/${i%.f*}_redun.sam.gz 2> /dev/null) | awk '{print $1}' | \
...
if [[ "$joint_calling" == false ]]; then
  ...
  if [[ $nodes -gt 1 ]] && test -f ${projdir}/GBSapp_run_node_1.sh; then cd /tmp/${samples_list%.txt}/preprocess/; fi
	...
	while IFS="" read -r i || [ -n "$i" ]; do (
		sleep $((RANDOM % 2))
    if [[ "$(ls ${projdir}/snpcall/${pop}_${ploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
  ...
  cd ${projdir}/preprocess/
	...
	if [[ "$samples_list" == "samples_list_node_1.txt" ]] && [[ "$(ls ${projdir}/snpcall/${pop}_${ploidy}x_raw.vcf* 2> /dev/null | wc -l)" -eq 0 ]]; then
    ...
    if [[ $call1 == $nodes ]]; then
      ...
      mv *.g.vcf* ./cohorts_1/
          ...
          if [[ -z "$interval_list" ]]; then
  					$GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" GenomicsDBImport ${input} -L ${selchr} --genomicsdb-workspace-path ${pop}_${ploidy}x_${selchr}_raw --genomicsdb-shared-posixfs-optimizations true --batch-size 50 --merge-input-intervals &&
  					wait
          else
            cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
          ...
          export TILEDB_DISABLE_FILE_LOCKING=1
            ...
            if [[ -z "$interval_list" ]]; then
              ...
              cat ${projdir}/${interval_list} | grep $selchr > ${projdir}/variant_intervals_${selchr}.list &&
              $GATK --java-options "$Xmxg -Djava.io.tmpdir=${projdir}/snpcall/tmp -XX:+UseParallelGC -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ${projdir}/refgenomes/$ref1 -L ${projdir}/variant_intervals_${selchr}.list -V gendb://${pop}_${ploidy}x_${selchr}_raw -O ${pop}_${ploidy}x_${selchr}_raw.hold.vcf.gz &&
              rm ${projdir}/variant_intervals_${selchr}.list &&
              rm -r ${pop}_${ploidy}x_${selchr}_raw &&
              mv ${pop}_${ploidy}x_${selchr}_raw.hold.vcf.gz ${pop}_${ploidy}x_${selchr}_raw.vcf.gz &&
              mv ${pop}_${ploidy}x_${selchr}_raw.hold.vcf.gz.tbi ${pop}_${ploidy}x_${selchr}_raw.vcf.gz.tbi &&
    ...
    if [[ -z "$p1" ]]; then
        ...
        if [[ "$lib_type" =~ "RRS" ]] || [[ "$lib_type" =~ "rrs" ]]; then
          ...
          if [[ -n "$RE1" ]]; then
            ...
            while IFS="" read -r i || [ -n "$i" ]; do
              sleep $((RANDOM % 2))
              $samtools view ../../preprocess/processed/${i%.f*}_${ref1%.f*}_precall.bam 2> /dev/null | awk '{print $3"\t"$4"\t"$10}' | awk -v seq="^${RE1}" '$3 ~ seq' | awk '{$2=sprintf("%d00",$2/100)}1' > ./seq_context_${RE1}/${i%.f*}_seqcontext.tmp &&
          ...
          if [[ -n "$RE2" ]]; then
            ...
            while IFS="" read -r i || [ -n "$i" ]; do
              sleep $((RANDOM % 2))
              $samtools view ../../preprocess/processed/${i%.f*}_${ref1%.f*}_precall.bam 2> /dev/null | awk '{print $3"\t"$4"\t"$10}' | awk -v seq="^${RE2}" '$3 ~ seq' | awk '{$2=sprintf("%d00",$2/100)}1' > ./seq_context_${RE2}/${i%.f*}_seqcontext.tmp &&
        ...
        if [[ "$lib_type" =~ "RRS" ]] || [[ "$lib_type" =~ "rrs" ]]; then
          ...
          if [[ -n "$RE1" ]]; then
            ...
            while IFS="" read -r i || [ -n "$i" ]; do
              sleep $((RANDOM % 2))
              $samtools view ../../preprocess/processed/${i%.f*}_${ref1%.f*}_precall.bam 2> /dev/null | awk '{print $3"\t"$4"\t"$10}' | awk -v seq="^${RE1}" '$3 ~ seq' | awk '{$2=sprintf("%d00",$2/100)}1' | \
          ...
          if [[ -n "$RE2" ]]; then
            ...
            while IFS="" read -r i || [ -n "$i" ]; do
              sleep $((RANDOM % 2))
              $samtools view ../../preprocess/processed/${i%.f*}_${ref1%.f*}_precall.bam 2> /dev/null | awk '{print $3"\t"$4"\t"$10}' | awk -v seq="^${RE2}" '$3 ~ seq' | awk '{$2=sprintf("%d00",$2/100)}1' | \
...
for snpfilter_dir in */; do (
	if [ -d "$snpfilter_dir" ]; then
      ...
      $bcftools view -s "$arr2" "${i%_dose.txt}".vcf > tmp.vcf && mv tmp.vcf ${i%_dose.txt}.vcf &&
			...
			Rscript "${GBSapp_dir}"/scripts/R/recode_vcf.R "${i%_dose.txt}_tmp.vcf" "$i" "${i%.txt}_AR_metric.txt" "${ploidy}x" "$darr" "${GBSapp_dir}/tools/R" &&
