...
if [ -z "$maxn_popallele" ]; then
	maxn_popallele=500 && SB=15000
fi
if [[ "$maxn_popallele" -gt 300 ]] && [[ "$maxn_popallele" -le 400 ]]; then
	SB=11000
fi
if [[ "$maxn_popallele" -gt 200 ]] && [[ "$maxn_popallele" -le 300 ]]; then
	SB=8000
fi
if [[ "$maxn_popallele" -gt 100 ]] && [[ "$maxn_popallele" -le 200 ]]; then
	SB=5000
fi
if [[ "$maxn_popallele" -gt 50 ]] && [[ "$maxn_popallele" -le 100 ]]; then
	SB=2500
fi
...
for i in $(ls -S *.f* | grep -v R2.f); do (
  ...
  awk -F ${ref4%.f*}_ '{print NF-1, NR}' ../preprocess/${i%.f*}_del.sam > ../preprocess/${i%.f*}_${ref4%.f*}_count.txt  && \
  paste <(awk '{print $1}' ../preprocess/${i%.f*}_${ref1%.f*}_count.txt ) <(awk '{print $1}' ../preprocess/${i%.f*}_${ref2%.f*}_count.txt ) <(awk '{print $1}' ../preprocess/${i%.f*}_${ref3%.f*}_count.txt ) <(awk '{print $1}' ../preprocess/${i%.f*}_${ref4%.f*}_count.txt ) > ../preprocess/${i%.f*}_pancount.txt && \
  ...
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk '{print $1"\t"$2"\t"$3"\t"$4}' | awk 'BEGIN {FS=OFS="\t"} {sum=0; n=0; for(i=1;i<=NF;i++) {sum+=$i; ++n} print sum}' |\
  ...
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 >= 1) && ($1 <= paralogs) && ($2 == 0) && ($3 == 0) && ($4 == 0)) { print } }'  >> ../preprocess/${i%.f*}_3del.sam   && \
  ...
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref4%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  awk '{if ($5=="@HD" || $5=="@SQ" || $5=="@PG") {print}}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 >= 1) && ($1 <= paralogs) && ($2 >= 1) && ($2 <= paralogs) && ($3 == 0) && ($4 == 0)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
  ...
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}_${ref4%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref2%.f*}_${ref4%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref3%.f*}_${ref4%.f*}.sam | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  awk '{if ($5=="@HD" || $5=="@SQ" || $5=="@PG") {print}}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 >= 1) && ($1 <= paralogs) && ($2 >= 1) && ($2 <= paralogs) && ($3 >= 1) && ($3 <= paralogs) && ($4 == 0)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
  ...
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}.sam  | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}.sam  | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}.sam  | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  awk '{if ($5=="@HD" || $5=="@SQ" || $5=="@PG") {print}}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if (($1 >= 1) && ($1 <= paralogs) && ($2 >= 1) && ($2 <= paralogs) && ($3 >= 1) && ($3 <= paralogs) && ($4 >= 1) && ($4 <= paralogs)) { print } }' >> ../preprocess/${i%.f*}_3del.sam   && \
  awk '!($1=$2=$3=$4="")' ../preprocess/${i%.f*}_3del.sam | awk '{$1=$1};1' > ../preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}.sam  && \
  awk '{if ($1=="@HD" || $1=="@SQ") {print}}' ../preprocess/${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}.sam  | awk '{gsub(/ /,"\t"); print}' > ../preprocess/${i%.f*}_heading.sam
  ...
  "${i%.f*}_${ref1%.f*}_${ref2%.f*}.sam" "${i%.f*}_${ref1%.f*}_${ref3%.f*}.sam" "${i%.f*}_${ref1%.f*}_${ref4%.f*}.sam" "${i%.f*}_${ref2%.f*}_${ref3%.f*}.sam" "${i%.f*}_${ref2%.f*}_${ref4%.f*}.sam" "${i%.f*}_${ref3%.f*}_${ref4%.f*}.sam" \
  "${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.sam" "${i%.f*}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}.sam" "${i%.f*}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}.sam" "${i%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}.sam" \
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy $ploidy -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 --max-alternate-alleles 8 )&
		...
		wait
		cd ../snpcall
		ls ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy $ploidy -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0 --max-alternate-alleles 8
	fi
...
if [ "$ncohorts" == no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1, 2, and 3 ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	j=-I; input=""; k=""
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1, 2, and 4 ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	j=-I; input=""; k=""
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1, 3, and 4 ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	j=-I; input=""; k=""
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 2, 3, and 4 ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	j=-I; input=""; k=""
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1 and 4 ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	j=-I; input=""; k=""
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 2 and 4 ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	j=-I; input=""; k=""
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 3 and 4 ${white}\n"
	cd $projdir
	cd preprocess
	mkdir processed
	j=-I; input=""; k=""
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${calcploidy} -O ../snpcall/${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${ploidy_ref1} -O ../snpcall/${pop}_${ref1%.f*}_${ploidy_ref1}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${ploidy_ref1} -O ../snpcall/${pop}_${ref1%.f*}_${ploidy_ref1}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${ploidy_ref2} -O ../snpcall/${pop}_${ref2%.f*}_${ploidy_ref2}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${ploidy_ref2} -O ../snpcall/${pop}_${ref2%.f*}_${ploidy_ref2}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${ploidy_ref3} -O ../snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref3%.f*}_${ploidy_ref3}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${ploidy_ref3} -O ../snpcall/${pop}_${ref3%.f*}_${ploidy_ref3}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" == no ]; then
	...
	if [[ "$Get_Chromosome" -gt 2000 ]]; then
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L $selchr $input -ploidy ${ploidy_ref4} -O ../snpcall/${pop}_${ref4%.f*}_${ploidy_ref4}x_"${selchr}"_raw.vcf.gz --max-reads-per-alignment-start 0 )&
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
		done
		wait
		cd ../snpcall
		ls ${pop}_${ref4%.f*}_${ploidy_ref4}x_*_raw.vcf.gz | parallel gunzip
		...
		$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta $input -ploidy ${ploidy_ref4} -O ../snpcall/${pop}_${ref4%.f*}_${ploidy_ref4}x_raw.vcf.gz --max-reads-per-alignment-start 0
	fi
	cd ${projdir}/preprocess
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling across entire genome (subgenomes 1, 2, 3 and 4) ${white}\n"
	mv ../snpcall/transit/*_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		...
		for i in $(ls -S *_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads"  HaplotypeCaller -R ../refgenomes/panref.fasta -I $i -ploidy $ploidy -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start 0 --max-alternate-alleles 8
	...
	for i in *.g.vcf.gz; do
		zcat $i | grep -v '^#' | awk '{print $1"__"$2"\t"$5}' >> allraw1.vcf
	done
	sort -k1,1 allraw1.vcf | awk '{gsub(/<NON_REF>/,""); print $0}' | \
	awk 'BEGIN{FS="\t"} {for(i=2; i<=NF; i++) { if (!a[$1]) a[$1]=$1FS$i ;else a[$1]=a[$1]""$i};if ($1 != old) b[j++] = a[old];old=$1 } END{for (i=0; i<j; i++) print b[i] }' | \
	awk -F\, '{print NF-1"\t"$0}' | awk -F "\t" 'NR>1{print $2"\t"$1}' | \
	awk '{print $1"\t"$2}' > highallelic1.vcf

	for i in *.g.vcf.gz; do
		zcat $i | grep -v '^#' | awk '{print $1"__"$2"\t"$10}' | awk '{gsub(/:/,"\t",$2); print}' | awk '{gsub(/ /,"\t"); print $1"\t"$6}' >> allraw2.vcf
	done
	sort -k1,1 allraw2.vcf | awk '{gsub(/<NON_REF>/,""); print $0}' | \
	awk 'BEGIN{FS="\t"} {for(i=2; i<=NF; i++) { if (!a[$1]) a[$1]=$1FS$i ;else a[$1]=a[$1]""$i};if ($1 != old) b[j++] = a[old];old=$1 } END{for (i=0; i<j; i++) print b[i] }' | \
	awk -F\, '{print NF-1"\t"$0}' | awk -F "\t" 'NR>1{print $2"\t"$1}' | \
	awk '{print $1"\t"$2}' > highallelic2.vcf

	awk 'FNR==NR{a[$1]=$2;next} ($1 in a) {print $1,a[$1],$2}' highallelic1.vcf highallelic2.vcf | awk '{gsub(/__/,"\t"); print}' | \
	awk '{gsub(/ /,"\t"); print}' | awk -v maxn_popallele=$maxn_popallele -v SB=$SB '$3 > maxn_popallele || $4 > SB' | awk '{print $1"\t"$2}' > highallelic.vcf
	rm allraw1.vcf allraw2.vcf highallelic1.vcf highallelic2.vcf

	for i in *.g.vcf.gz; do
		zcat $i | awk 'NR==FNR{a[$0];next} !($0 in a)' highallelic.vcf - >  lowerallelic.g.vcf
		bgzip lowerallelic.g.vcf && mv  lowerallelic.g.vcf.gz ${i}
		tabix -p vcf ${i}
		wait
	done
	rm highallelic.vcf
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' )
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_"${selchr}"_raw.vcf.gz --max-alternate-alleles 8
			rm ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
			if LC_ALL=C gzip -l ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_"${selchr}"_raw.vcf.gz | awk 'NR==2 {exit($2!=0)}'; then
				...
				echo -e "${magenta}- \n- Set parameter maxn_popallele to lower value (default=500) or cohort_size to higher value (default=1) ${white}\n"
		...
		cd ../
		bgzip ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${ploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1, 2, and 3 ${white}\n"
	calcploidy=$(($ploidy_ref1+$ploidy_ref2+$ploidy_ref3))
	mv ../snpcall/transit/*_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} -v pat3=${ref3%.f*} 'pat1 || pat2 || pat3' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${calcploidy} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} -v pat3=${ref3%.f*} 'pat1 || pat2 || pat3')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref1%.f*}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1, 2, and 4 ${white}\n"
	calcploidy=$(($ploidy_ref1+$ploidy_ref2+$ploidy_ref4))
	mv ../snpcall/transit/*_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} -v pat3=${ref4%.f*} 'pat1 || pat2 || pat3' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${calcploidy} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} -v pat3=${ref4%.f*} 'pat1 || pat2 || pat3')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref1%.f*}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1, 3, and 4 ${white}\n"
	calcploidy=$(($ploidy_ref1+$ploidy_ref3+$ploidy_ref4))
	mv ../snpcall/transit/*_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref3%.f*} -v pat3=${ref4%.f*} 'pat1 || pat2 || pat3' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${calcploidy} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref3%.f*} -v pat3=${ref4%.f*} 'pat1 || pat2 || pat3')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref1%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 2, 3, and 4 ${white}\n"
	calcploidy=$(($ploidy_ref2+$ploidy_ref3+$ploidy_ref4))
	mv ../snpcall/transit/*_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref2%.f*} -v pat2=${ref3%.f*} -v pat3=${ref4%.f*} 'pat1 || pat2 || pat3' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${calcploidy} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref2%.f*} -v pat2=${ref3%.f*} -v pat3=${ref4%.f*} 'pat1 || pat2 || pat3')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref2%.f*}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} 'pat1 || pat2')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref1%.f*}_${ref2%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1 and 3 ${white}\n"
	calcploidy=$(($ploidy_ref1+$ploidy_ref3))
	mv ../snpcall/transit/*_${ref1%.f*}_${ref3%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} 'pat1 || pat2')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref1%.f*}_${ref3%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 1 and 4 ${white}\n"
	calcploidy=$(($ploidy_ref1+$ploidy_ref4))
	mv ../snpcall/transit/*_${ref1%.f*}_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}'| awk -v pat1=${ref1%.f*} -v pat2=${ref4%.f*} 'pat1 || pat2' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref1%.f*}_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${calcploidy} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} 'pat1 || pat2')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref1%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 2 and 3 ${white}\n"
	calcploidy=$(($ploidy_ref2+$ploidy_ref3))
	mv ../snpcall/transit/*_${ref2%.f*}_${ref3%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} 'pat1 || pat2')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref2%.f*}_${ref3%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 2 and 4 ${white}\n"
	calcploidy=$(($ploidy_ref2+$ploidy_ref4))
	mv ../snpcall/transit/*_${ref2%.f*}_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}'| awk -v pat1=${ref2%.f*} -v pat2=${ref4%.f*} 'pat1 || pat2' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref2%.f*}_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${calcploidy} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} 'pat1 || pat2')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref2%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenomes 3 and 4 ${white}\n"
	calcploidy=$(($ploidy_ref3+$ploidy_ref4))
	mv ../snpcall/transit/*_${ref3%.f*}_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}'| awk -v pat1=${ref3%.f*} -v pat2=${ref4%.f*} 'pat1 || pat2' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref3%.f*}_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${calcploidy} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat1=${ref1%.f*} -v pat2=${ref2%.f*} 'pat1 || pat2')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref3%.f*}_${ref4%.f*}_${calcploidy}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	...
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		...
		for i in $(ls -S *_${ref1%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${ploidy_ref1} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
...
if [ "$ncohorts" != no ]; then
	...
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		...
		for i in $(ls -S *_${ref2%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${ploidy_ref2} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenome-3 ${white}\n"
	mv ../snpcall/transit/*_${ref3%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		...
		for i in $(ls -S *_${ref3%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${ploidy_ref3} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref3%.f*} '$0 ~ pat')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref3%.f*}_${ploidy_ref3}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref3%.f*}_${ploidy_ref3}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref3%.f*}_${ploidy_ref3}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref3%.f*}_${ploidy_ref3}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref3%.f*}_${ploidy_ref3}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref3%.f*}_${ploidy_ref3}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref3%.f*}_${ploidy_ref3}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
...
if [ "$ncohorts" != no ]; then
	echo -e "${magenta}- performing SNP calling on subgenome-4 ${white}\n"
	mv ../snpcall/transit/*_${ref4%.f*}.g.vcf.gz* ../snpcall/
	if ls ../snpcall/*.g.vcf.gz >/dev/null 2>&1; then
		echo -e "${magenta}- skipping HaplotypeCaller. Using previously generated .g.vcf.gz files ${white}\n"
	else
		awk 'NR>1{print $2,"\t",$3}' ../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}'| awk -v pat=${ref4%.f*} '$0 ~ pat' > ../refgenomes/intervals.list
		for i in $(ls -S *_${ref4%.f*}_precall.bam); do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -L ../refgenomes/intervals.list -I $i -ploidy ${ploidy_ref4} -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --max-reads-per-alignment-start $maxRD
			cp ../snpcall/${i%_precall.bam}.g.vcf.gz* ../snpcall/processed/ ) &
			if [[ $(jobs -r -p | wc -l) -ge $gN ]]; then
				wait
			fi
	...
	for dir in cohorts*/; do
		...
		Get_Chromosome=$(awk 'NR>1{print $2,"\t",$3}' ../../refgenomes/panref.dict | awk '{gsub(/SN:/,"");gsub(/LN:/,""); print $0}' | sort -k2,2 -nr | awk '{print $1}' | awk -v pat=${ref4%.f*} '$0 ~ pat')
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" CombineGVCFs -R ../../refgenomes/panref.fasta -L $selchr $input -O ${pop}_${ref4%.f*}_${ploidy_ref4}x_"${selchr}"_raw.g.vcf.gz
			$GATK --java-options "$Xmx3 -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V ${pop}_${ref4%.f*}_${ploidy_ref4}x_"${selchr}"_raw.g.vcf.gz -O ${pop}_${ref4%.f*}_${ploidy_ref4}x_"${selchr}"_raw.vcf.gz
			rm ${pop}_${ref4%.f*}_${ploidy_ref4}x_"${selchr}"_raw.g.vcf.gz ${pop}_${ref4%.f*}_${ploidy_ref4}x_"${selchr}"_raw.g.vcf.gz.tbi
		...
		cd ../
		bgzip ${pop}_${ref4%.f*}_${ploidy_ref4}x_raw_${dir%/}.vcf
		tabix -p vcf ${pop}_${ref4%.f*}_${ploidy_ref4}x_raw_${dir%/}.vcf.gz
		wait
	done
	wait
	if [[ `ls -1 *cohorts*.vcf.gz 2>/dev/null | wc -l` -gt 1 ]]; then
		$bcftools merge *cohorts*.vcf.gz --force-samples -m all > ${pop}_${ref4%.f*}_${ploidy_ref4}x_raw.vcf
	else
...
######################################################################################################################################################
echo -e "${blue}\n############################################################################## ${yellow}\n- GBSapp is Performing Variant Filtering\n${blue}##############################################################################${white}\n"
main () {
cd $projdir
mkdir snpfilter
cd snpfilter
