...
for i in $(ls -S *.f* | grep -v R2.f); do (
  ...
  awk '{if ($3=="@HD" || $3=="@SQ" || $3=="@PG"); print $0}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if ($1 >= 1 && $1 <= paralogs && $2 == 0); print $0}' >> ../preprocess/${i%.f*}_3del.sam   && \
  ...
  awk '!/^@/ { print }' ../preprocess/${i%.f*}_${ref1%.f*}.sam | awk '{gsub(/^.*-/,"",$1); print $0}' | awk '{gsub(/^.*_/,"",$2); print $0}' > ../preprocess/${i%.f*}_uniq.sam
	...
	if [ "$multilocus" == "true" ]; then
		...
		grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk '{print $1"~"$7"~"$5"~"$6}' >> ../alignment_summaries/refgenome_paralogs_${ref1%.f*}.txt
  ...
  awk '{if ($3=="@HD" || $3=="@SQ" || $3=="@PG"); print $0}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if ($1 == 0 && $2 >= 1 && $2 <= paralogs); print $0}' >> ../preprocess/${i%.f*}_3del.sam   && \
  ...
  awk '!/^@/ { print }' ../preprocess/${i%.f*}_${ref2%.f*}.sam | awk '{gsub(/^.*-/,"",$1); print $0}' | awk '{gsub(/^.*_/,"",$2); print $0}' > ../preprocess/${i%.f*}_uniq.sam
	...
	if [ "$multilocus" == "true" ]; then
		...
		grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk '{print $2"~"$7"~"$5"~"$6}' >> ../alignment_summaries/refgenome_paralogs_${ref2%.f*}.txt
  ...
  awk '{if ($3=="@HD" || $3=="@SQ" || $3=="@PG"); print $0}' ../preprocess/${i%.f*}_2del.sam > ../preprocess/${i%.f*}_3del.sam  && \
  grep -vwE "(@HD|@SQ|@PG)" ../preprocess/${i%.f*}_2del.sam | awk -v paralogs=$paralogs '{ if ($1 >= 1 && $1 <= paralogs && $2 >= 1 && $2 <= paralogs); print $0}' >> ../preprocess/${i%.f*}_3del.sam   && \
...
######################
	...
	for i in $(ls -S *_${ref1%.f*}_${ref2%.f*}_precall.bam); do (
		$GATK --java-options "$Xmxg -XX:ParallelGCThreads=$gthreads" HaplotypeCaller -R ../refgenomes/panref.fasta -I $i -ploidy $ploidy -O ../snpcall/${i%_precall.bam}.g.vcf.gz -ERC GVCF --dont-use-soft-clipped-bases $softclip --max-reads-per-alignment-start "$(( biased_downsample * ploidy ))" --max-num-haplotypes-in-population "$((ploidy * maxHaplotype))"
		) &
	...
	for dir in cohorts*/; do
		...
		for selchr in $Get_Chromosome; do (
			$GATK --java-options "$Xmxg -XX:ParallelGCThreads=$gthreads" GenomicsDBImport $input -L $selchr --genomicsdb-workspace-path ${pop}_${ref1%.f*}_${ref2%.f*}_${ploidy}x_"${selchr}"_raw
			$GATK --java-options "$Xmxg -XX:ParallelGCThreads=$gthreads" GenotypeGVCFs -R ../../refgenomes/panref.fasta -L $selchr -V gendb://${pop}_${ref1%.f*}_${ref2%.f*}_${ploidy}x_"${selchr}"_raw -O ${pop}_${ref1%.f*}_${ref2%.f*}_${ploidy}x_"${selchr}"_raw.vcf.gz
