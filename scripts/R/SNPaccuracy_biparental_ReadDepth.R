...
path <- getwd()
path <- paste(path,"/",sep="")
file.names <- dir(path,pattern ="noSD.*\\.txt",full.names=TRUE)
for(i in 1:length(file.names)){
  all_content <- readLines(paste(file.names[i],sep=""))
  noSD <- read.table(textConnection(all_content), header = TRUE, check.names = FALSE)
  noSD <- merge(noSD, keep, by="SNP")
  write.table (noSD, file=paste(file.names[i]), row.names=F, quote = FALSE, sep = "\t")
}
...
if (maxdose==2) {
  ...
  for (i in 1:nrow(snplist)) {
    ...
    plot <- ggplot(genoaccu, aes(x=ref, y=alt), group="Genotype") +
      ...
      annotate("point", x=h1[1], y=h1[2], size=12, color="grey50", pch="*") + coord_equal() +
      geom_text(label=paste("\np1: ",p1,"\n",sep=""), x=h1[1], y=h1[2], vjust ="inward", hjust="inward", color="grey50", size=4, fontface='italic') +
      annotate("point", x=h2[1], y=h2[2], size=12, color="grey50", pch="*") + 
