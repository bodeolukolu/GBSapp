...
if (ploidy == "2x"){
  ...
  ARplot <- ggplot(AR,aes(x=propHet,y=Allele_Ratio))+
    ...
    annotate("rect", xmin=0, xmax=1, ymin=-0.1, ymax=0.1, alpha=0.2, fill="tomato")+
...
if (ploidy == "6x"){
  AR$propHet <- (rowSums(AR[,5:(GTlen+4)] == "1" | AR[,5:(GTlen+4)] == "2" | AR[,5:(GTlen+4)] == "3" | AR[,5:(GTlen+4)] == "4" | AR[,5:(GTlen+4)] == "5", na.rm = TRUE)) / (GTlen - (apply(AR[,5:(GTlen+4)], 1, function(x) sum(is.na(x)))))
  AR <- AR[,c(keepAR)]
  AR <- subset(AR, AR$propHet != 0)
  AR[,1:(ncol(AR)-1)][AR[,1:(ncol(AR)-1)] == 0] <- NA
  AR <- AR[,c(which(colnames(AR)=="propHet"),which(colnames(AR)!="propHet"))]
...
if (ploidy == "8x"){  
  AR$propHet <- (rowSums(AR[,5:(GTlen+4)] == "1" | AR[,5:(GTlen+4)] == "2" | AR[,5:(GTlen+4)] == "3" | AR[,5:(GTlen+4)] == "4" | AR[,5:(GTlen+4)] == "5" | AR[,5:(GTlen+4)] == "6" | AR[,5:(GTlen+4)] == "7", na.rm = TRUE)) / (GTlen - (apply(AR[,5:(GTlen+4)], 1, function(x) sum(is.na(x)))))
  AR <- AR[,c(keepAR)]
  AR[,1:(ncol(AR)-1)][AR[,1:(ncol(AR)-1)] == 0] <- NA
  AR <- AR[,c(which(colnames(AR)=="propHet"),which(colnames(AR)!="propHet"))]
  AR <- suppressWarnings(na.omit(cbind(AR[1], stack(AR[-1]), row.names = NULL)))
  ...
  ARplot <- ggplot(AR,aes(x=propHet,y=Allele_Ratio))+
    geom_point(aes(x = propHet, y = Allele_Ratio, color = Allele_Ratio), size = 1, pch=19, alpha=0.1)+
    geom_density_2d(bins=50)+
    annotate("rect", xmin=0, xmax=1, ymin=-0.03, ymax=0.2, alpha=0.03, fill="tomato")+
    scale_x_continuous(expand=c(0,0))+
    scale_y_continuous(expand=c(0,0))+
    scale_colour_gradient2(low="darkorange3", mid="darkgoldenrod1", high ="cornflowerblue",
                           breaks=c(0.75,-0.75), limits=c(-1,1), 
